...
CONTAINER_NAME="erp4-knowledge-llm-budget-$$"
...
migration_diff="$(npx --prefix "$ROOT_DIR/packages/backend" prisma migrate diff \
  ...
  --script)"
if grep -Eq 'KnowledgeLlm' <<<"$migration_diff"; then
  echo 'Knowledge LLM migration/schema drift detected:' >&2
  grep -E 'KnowledgeLlm' <<<"$migration_diff" >&2
  exit 1
fi
...
echo "knowledge LLM budget PostgreSQL integration: PASS"
