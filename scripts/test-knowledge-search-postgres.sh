...
CONTAINER_NAME="erp4-knowledge-search-test-$$"
...
TEST_DATABASE="erp4_knowledge_search_test"
TEST_USER="erp4_search_test"
...
export KNOWLEDGE_SEARCH_INTEGRATION_CONFIRM=1
...
node "$ROOT_DIR/packages/backend/scripts/knowledge-search-integration.mjs"
npx --prefix "$ROOT_DIR/packages/backend" prisma migrate status \
  --config "$ROOT_DIR/packages/backend/prisma.config.ts" >/dev/null

echo "knowledge search PostgreSQL integration: PASS"
