...
BASE_SHA="${KNOWLEDGE_THREAD_PROMOTION_OLD_APP_BASE_SHA:-b720f575140e533c59070d33723146925faad489}"
EXPECTED_BASE_SHA="b720f575140e533c59070d33723146925faad489"
...
CONTAINER_NAME="erp4-knowledge-thread-promotion-old-app-$$"
SCRATCH_ROOT="$ROOT_DIR/.codex-local/tmp/knowledge-thread-promotion-old-app-$$"
OLD_APP_ROOT="$SCRATCH_ROOT/old-app"
TEST_DATABASE="erp4_knowledge_thread_promotion_old_app_test"
TEST_USER="erp4_knowledge_thread_promotion_old_app_test"
TEST_PASSWORD="$({
  node -e 'process.stdout.write(require("node:crypto").randomBytes(24).toString("hex"))'
})"

if [[ "$BASE_SHA" != "$EXPECTED_BASE_SHA" ]]; then
  echo "Refusing an unreviewed Knowledge thread promotion old-app compatibility baseline" >&2
  exit 1
fi
...
cleanup() {
  ...
  if [[ -d "$SCRATCH_ROOT" && "$SCRATCH_ROOT" == "$ROOT_DIR/.codex-local/tmp/knowledge-thread-promotion-old-app-"* ]]; then
...
export KNOWLEDGE_CURSOR_SIGNING_SECRET="knowledge-thread-promotion-old-app-compat-secret-0001"
export KNOWLEDGE_SHARE_OLD_APP_CONFIRM=1
export KNOWLEDGE_THREAD_PROMOTION_OLD_APP_CONFIRM=1
export KNOWLEDGE_THREAD_PROMOTION_SCHEMA_INTEGRATION_CONFIRM=1
export TZ=UTC
export KNOWLEDGE_THREAD_PROMOTION_OLD_APP_BASE_SHA="$BASE_SHA"
export OLD_APP_ROOT
...
export KNOWLEDGE_SHARE_OLD_APP_MODE=seed
node "$ROOT_DIR/packages/backend/scripts/knowledge-thread-promotion-old-app-compat.mjs"
...
export KNOWLEDGE_SHARE_OLD_APP_MODE=current-create
node "$ROOT_DIR/packages/backend/scripts/knowledge-thread-promotion-old-app-compat.mjs"
npm run build --prefix "$ROOT_DIR/packages/backend" >/dev/null
node "$ROOT_DIR/packages/backend/scripts/knowledge-thread-promotion-application-integration.mjs" >/dev/null

export KNOWLEDGE_SHARE_OLD_APP_MODE=old-after
node "$ROOT_DIR/packages/backend/scripts/knowledge-thread-promotion-old-app-compat.mjs"
export KNOWLEDGE_SHARE_OLD_APP_MODE=current-after
node "$ROOT_DIR/packages/backend/scripts/knowledge-thread-promotion-old-app-compat.mjs"

printf '{"baseline":"%s","oldMigrations":%s,"newMigrations":%s,"chatMessageType":"text","promotionActualRow":true,"oldApplicationReadWrite":true,"healthReadiness":true}\n' \
  "$BASE_SHA" "$OLD_MIGRATIONS" "$NEW_MIGRATIONS"
echo "knowledge thread promotion old-application compatibility: PASS"
