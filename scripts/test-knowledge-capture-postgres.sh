...
CONTAINER_NAME="erp4-knowledge-capture-test-$$"
...
TEST_DATABASE="erp4_knowledge_capture_test"
TEST_USER="erp4_knowledge_capture_test"
...
export KNOWLEDGE_CAPTURE_INTEGRATION_CONFIRM=1
...
migration_diff="$(npx --prefix "$ROOT_DIR/packages/backend" prisma migrate diff \
  ...
  --script)"
if grep -Eq 'KnowledgeCapture' <<<"$migration_diff"; then
  echo 'Knowledge capture migration/schema drift detected:' >&2
  grep -E 'KnowledgeCapture' <<<"$migration_diff" >&2
  exit 1
fi
npm run build --prefix "$ROOT_DIR/packages/backend" >/dev/null
node "$ROOT_DIR/packages/backend/scripts/knowledge-capture-integration.mjs"
npx --prefix "$ROOT_DIR/packages/backend" prisma migrate status \
  --config "$ROOT_DIR/packages/backend/prisma.config.ts" >/dev/null

echo "knowledge capture PostgreSQL integration: PASS"
