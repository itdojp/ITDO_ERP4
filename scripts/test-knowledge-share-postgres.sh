...
CONTAINER_NAME="erp4-knowledge-share-test-$$"
...
TEST_DATABASE="erp4_knowledge_share_test"
TEST_USER="erp4_knowledge_share_test"
...
export KNOWLEDGE_SHARE_INTEGRATION_CONFIRM=1
...
migration_diff="$(npx --prefix "$ROOT_DIR/packages/backend" prisma migrate diff \
  ...
  --script)"
if grep -Eq 'KnowledgeShare|KnowledgeShareStatus|KnowledgeShareFailureCode' <<<"$migration_diff"; then
  echo 'Knowledge share migration/schema drift detected:' >&2
  grep -E 'KnowledgeShare|KnowledgeShareStatus|KnowledgeShareFailureCode' <<<"$migration_diff" >&2
  exit 1
fi
npm run build --prefix "$ROOT_DIR/packages/backend" >/dev/null
node "$ROOT_DIR/packages/backend/scripts/knowledge-share-integration.mjs"
npx --prefix "$ROOT_DIR/packages/backend" prisma migrate status \
  --config "$ROOT_DIR/packages/backend/prisma.config.ts" >/dev/null

echo "knowledge share PostgreSQL integration: PASS"
