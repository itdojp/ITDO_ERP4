...
CONTAINER_NAME="erp4-knowledge-thread-promotion-schema-$$"
...
TEST_DATABASE="erp4_knowledge_thread_promotion_schema"
TEST_USER="erp4_knowledge_thread_promotion_schema"
...
migration_diff="$(npx --prefix "$ROOT_DIR/packages/backend" prisma migrate diff \
  ...
  --script)"
promotion_drift_pattern='KnowledgeThreadPromotion|KnowledgeSynthesisGroupGrant|sourceThreadPromotionId'
if grep -Eq "$promotion_drift_pattern" <<<"$migration_diff"; then
  echo 'Knowledge thread promotion migration/schema drift detected:' >&2
  grep -E "$promotion_drift_pattern" <<<"$migration_diff" >&2
  exit 1
fi

node "$ROOT_DIR/packages/backend/scripts/knowledge-thread-promotion-schema-integration.mjs"
node "$ROOT_DIR/packages/backend/scripts/knowledge-thread-promotion-application-integration.mjs"
npx --prefix "$ROOT_DIR/packages/backend" prisma migrate status \
  --config "$ROOT_DIR/packages/backend/prisma.config.ts" >/dev/null

echo "knowledge thread promotion schema PostgreSQL integration: PASS"
