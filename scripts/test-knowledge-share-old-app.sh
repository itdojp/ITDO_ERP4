...
BASE_SHA="${KNOWLEDGE_SHARE_OLD_APP_BASE_SHA:-421b38e82fa545e46348c49d0646531f8ddc4cd2}"
...
CONTAINER_NAME="erp4-knowledge-share-old-app-$$"
SCRATCH_ROOT="$ROOT_DIR/.codex-local/tmp/knowledge-share-old-app-$$"
OLD_APP_ROOT="$SCRATCH_ROOT/old-app"
TEST_DATABASE="erp4_knowledge_share_old_app_test"
TEST_USER="erp4_knowledge_share_old_app_test"
TEST_PASSWORD="$({
  node -e 'process.stdout.write(require("node:crypto").randomBytes(24).toString("hex"))'
})"

if [[ "$BASE_SHA" != "$EXPECTED_BASE_SHA" ]]; then
  echo "Refusing an unreviewed Knowledge share old-app compatibility baseline" >&2
  exit 1
fi
...
cleanup() {
  ...
  if [[ -d "$SCRATCH_ROOT" && "$SCRATCH_ROOT" == "$ROOT_DIR/.codex-local/tmp/knowledge-share-old-app-"* ]]; then
...
export KNOWLEDGE_CURSOR_SIGNING_SECRET="knowledge-share-old-app-compat-secret-0001"
export KNOWLEDGE_SHARE_OLD_APP_CONFIRM=1
export KNOWLEDGE_SHARE_OLD_APP_BASE_SHA="$BASE_SHA"
export OLD_APP_ROOT
...
node "$ROOT_DIR/packages/backend/scripts/knowledge-share-old-app-compat.mjs"
...
printf '{"baseline":"%s","oldMigrations":%s,"newMigrations":%s,"chatMessageType":"text","shareSideTable":true,"oldApplicationReadWrite":true,"healthReadiness":true}\n' \
  "$BASE_SHA" "$OLD_MIGRATIONS" "$NEW_MIGRATIONS"
echo "knowledge share old-application compatibility: PASS"
