...
test('phase 8 PDF files UX/UI summary renders @core', async ({ page }) => {
  ...
  await expect(section.locator('[aria-label="PDF管理サマリー"]')).toBeVisible({
    timeout: actionTimeout,
  });
  await expect(
    section.getByRole('heading', { name: 'PDF検索とファイル確認' }),
  ).toBeVisible({ timeout: actionTimeout });
  await expect(section.getByLabel('filename prefix')).toBeVisible({
    timeout: actionTimeout,
  });
  await expect(section.getByRole('button', { name: '再読込' })).toBeVisible({
    timeout: actionTimeout,
  });

  await captureSection(section, '01-uiux-pdf-files.png');
});
