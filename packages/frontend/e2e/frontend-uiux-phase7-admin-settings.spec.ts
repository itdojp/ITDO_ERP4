...
test('phase 7 admin settings UX/UI summary renders @core', async ({ page }) => {
  ...
  await expect(section.locator('[aria-label="設定管理サマリー"]')).toBeVisible({
    timeout: actionTimeout,
  });
  await expect(
    section.getByRole('heading', { name: 'コミュニケーション・組織' }),
  ).toBeVisible({ timeout: actionTimeout });
  await expect(
    section.getByRole('heading', { name: '承認・権限ポリシー' }),
  ).toBeVisible({ timeout: actionTimeout });
  await expect(
    section.getByRole('heading', { name: '外部連携・会計連携' }),
  ).toBeVisible({ timeout: actionTimeout });

  await captureSection(section, '01-uiux-admin-settings.png');
});
