...
test('frontend auth gateway bff local login shows MFA setup required guidance @extended', async ({
  page,
}) => {
  test.skip(!isBffMode, 'jwt_bff build only');
  ensureEvidenceDir();
  ...
  await page.route('**/auth/local/login', async (route) => {
    ...
    await route.fulfill({
      ...
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        error: { code: 'local_mfa_setup_required' },
      }),
    });
  });
  ...
  await expect(
    page.getByText('MFA 設定が必要です。system_admin に依頼してください'),
  ).toBeVisible();
});

test('frontend auth gateway bff local login shows locked guidance @extended', async ({
  page,
}) => {
  test.skip(!isBffMode, 'jwt_bff build only');
  ensureEvidenceDir();
...
test('frontend auth gateway bff local login shows rate limit guidance @extended', async ({
  page,
}) => {
  test.skip(!isBffMode, 'jwt_bff build only');
  ensureEvidenceDir();
...
test('frontend auth gateway bff clears local auth state when current session is revoked @extended', async ({
  page,
}) => {
  test.skip(!isBffMode, 'jwt_bff build only');
  ...
  await expect(page.getByText('ID: bff-user')).toHaveCount(0);
  await expect(page.getByRole('button', { name: 'Googleでログイン' })).toBeVisible();
  await expect(page.getByText('認証セッション')).toHaveCount(0);
  await expect(
    page.getByText('現在の認証セッションを終了しました。再度ログインしてください'),
  ).toHaveCount(0);
});
...
test('frontend auth gateway bff retries session revoke after invalid csrf token @extended', async ({
  ...
  await page.route('**/auth/csrf', async (route) => {
    ...
    await route.fulfill({
      ...
      headers: {
        'content-type': 'application/json',
        'set-cookie': `erp4_csrf=${csrfToken}; Path=/; HttpOnly; SameSite=Strict`,
      },
  ...
  await page.route('**/auth/sessions/sess-other/revoke', async (route) => {
    ...
    if (revokeAttemptCount === 1) {
      ...
      await route.fulfill({
        status: 403,
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ error: { code: 'invalid_csrf_token' } }),
      });
      return;
    }
  ...
  expect(revokeAttemptCount).toBe(2);
});
