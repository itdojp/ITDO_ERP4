...
const baseline = process.env.KNOWLEDGE_THREAD_PROMOTION_OLD_APP_BASE_SHA;
...
if (
  process.env.KNOWLEDGE_THREAD_PROMOTION_OLD_APP_CONFIRM !== '1' ||
  baseline !== 'b720f575140e533c59070d33723146925faad489' ||
  !oldRoot ||
  !currentRoot ||
  !['seed', 'current-create', 'old-after', 'current-after'].includes(mode) ||
  !databaseUrl ||
  !['127.0.0.1', 'localhost'].includes(databaseUrl.hostname) ||
  databaseUrl.pathname !== '/erp4_knowledge_thread_promotion_old_app_test'
) {
  throw new Error(
    'Refusing to run outside the confirmed loopback Knowledge thread promotion old-app database',
  );
}
