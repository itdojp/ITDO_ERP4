...
import {
  reportDeliveryDue,
  reportGroupEffort,
  reportManagementAccountingSummary,
  reportOvertime,
...
function buildManagementAccountingSummaryCsv(
  ...
  const rows: Array<Array<string | number>> = [
    [
      ...
      toNullableCsvNumber(summary.payrollConfirmedLaborCost),
      toNullableCsvNumber(summary.laborCostVariance),
      sanitizeCsvCell(summary.payrollConfirmedStatus),
  ...
  for (const item of summary.currencyBreakdown) {
    rows.push([
      ...
      item.directCost,
      item.laborCost,
      toNullableCsvNumber(item.payrollConfirmedLaborCost),
      toNullableCsvNumber(item.laborCostVariance),
      '',
      item.vendorCost,
      item.expenseCost,
      item.grossProfit,
  ...
  for (const item of summary.departmentBreakdown) {
    rows.push([
      ...
      sanitizeCsvCell(item.departmentExternalCode ?? ''),
      sanitizeCsvCell(item.departmentSource),
      '',
      '',
      '',
      item.projectCount,
  ...
  for (const item of summary.currencyBreakdown) {
    for (const project of item.topRedProjects) {
      rows.push([
        ...
        sanitizeCsvCell(project.departmentName ?? ''),
        sanitizeCsvCell(project.departmentExternalCode ?? ''),
        sanitizeCsvCell(project.departmentSource),
        ...
        project.directCost,
        project.laborCost,
        toNullableCsvNumber(project.payrollConfirmedLaborCost),
        toNullableCsvNumber(project.laborCostVariance),
        '',
        project.vendorCost,
        project.expenseCost,
        project.grossProfit,
