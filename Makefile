...
bounded-context-coverage-check:
	npm run arch:bounded-context:coverage --prefix packages/backend

knowledge-provenance-postgres:
	./scripts/test-knowledge-provenance-postgres.sh

knowledge-provenance-old-app:
	./scripts/test-knowledge-provenance-old-app.sh

knowledge-conversation-import-postgres:
	./scripts/test-knowledge-conversation-import-postgres.sh

chat-thread-postgres:
	./scripts/test-chat-thread-postgres.sh

chat-thread-old-app:
	./scripts/test-chat-thread-old-app.sh
...
backup-s3-prune-plan:
	@test -n "$(PLAN_JSON)" -a -n "$(PLAN_MARKDOWN)" || (echo 'PLAN_JSON and PLAN_MARKDOWN are required' >&2; exit 1)
	./scripts/backup-s3-retention.sh --dry-run --plan-json "$(PLAN_JSON)" --plan-markdown "$(PLAN_MARKDOWN)"
...
production-readiness-external-evidence-check:
	node scripts/check-production-readiness-external-evidence.mjs

production-readiness-external-evidence-test:
	node --test scripts/check-production-readiness-external-evidence.test.mjs
