...
既定では毎日 03:45 に `erp4-config-prune.service` が実行され、`prune-backups.sh --keep-count "${ERP4_BACKUP_KEEP_COUNT:-14}" --keep-days "${ERP4_BACKUP_KEEP_DAYS:-30}"` 相当の整理を行います。保持本数と保持日数は `erp4-maintenance.env` の `ERP4_BACKUP_KEEP_COUNT` / `ERP4_BACKUP_KEEP_DAYS` で上書きできます。
