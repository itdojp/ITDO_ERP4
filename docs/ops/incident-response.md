...
### 重要ジョブ失敗
- 対象ジョブ名、失敗回数、直近成功時刻を確認
- 「重要ジョブ」はユーザ影響や外部通知に直結するジョブ（例: `security-audit`、通知送信系ジョブ）を指す。判断に迷う場合は `docs/ops/alerting.md` / `docs/ops/slo.md` を参照
- 失敗ログと外部依存（SMTP / webhook / queue など）の状態を確認
- 直近デプロイSHA / 設定変更 / secret rotation の有無を確認

## 対応中の記録（推奨）
対応が長引く場合、以下を必ず残す。通知テンプレートから転記できる項目は同じ語彙で残す。
- 発生時刻/検知時刻
- 暫定 SEV と判断理由
- 影響範囲（誰が/何ができないか）
- 直近変更（デプロイSHA/tag、Feature Flag変更など）
- request-id（例: `x-request-id`）
- `/healthz` / `/readyz` の結果
- 実施した対応と結果（時系列）
- 次のエスカレーション先または Issue / PR 参照
