...
## 重要度（暫定）
本ドキュメントでは `docs/ops/incident-response.md` と同じ SEV 定義を使う。SEV の最終判断は「影響」と「緊急度」で行い、原因種別では判断しない。
- SEV1: 広範囲/重大（業務停止相当、または情報セキュリティ事故の疑い）
- SEV2: 主要導線に影響（回避策あり、放置できない）
- SEV3: 限定的影響（運用で吸収可能、営業時間内に対応）
