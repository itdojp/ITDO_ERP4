...
- Repo-side readiness runner が対象コミットの clean checkout で成功（release Go の正式 repo-side 証跡は `RELEASE_E2E_SCOPE=full make release-readiness-record`。正式証跡を作らない限定確認では `RELEASE_E2E_SCOPE=core make release-readiness`）
...
- 対象コミットの clean checkout で `RELEASE_E2E_SCOPE=full make release-readiness-record` を実行し、`docs/test-results/YYYY-MM-DD-release-readiness-rN.md` に release Go の正式 repo-side readiness 証跡を作成する。`tmp/release-readiness/*/summary.md` は限定・調査用証跡に留める。runner の `CI job` 欄は対応先の参照であり、GitHub Actions の実結果はPRまたは対象コミットで別途確認する
