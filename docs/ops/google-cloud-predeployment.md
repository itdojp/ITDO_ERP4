...
| provider switch     | contextごとの`CHAT_ATTACHMENT_PROVIDER` / `PDF_PROVIDER` / `EVIDENCE_ARCHIVE_PROVIDER` / `REPORT_PROVIDER` |

#1977はPDF、Evidence archive、Reportのfolder preflight、storage port、認可済みdownload endpoint、runtime provider実装を提供する。ただし実Google Driveでのread/write、既存local artifactのcopy-only照合、rollback windowは未完了である。#1981のcutover承認前に`PDF_PROVIDER=gdrive`、`EVIDENCE_ARCHIVE_PROVIDER=gdrive`、`REPORT_PROVIDER=gdrive`をproduction envへ反映しない。
