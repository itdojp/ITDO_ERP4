...
- #1977 foundationではcopy-only helperと共通artifact lifecycleまでを提供する。runtimeのgdrive provider切替は#1977最終PRの適用前に行わない
...
Storage artifact migration（#1977 foundation）:

- `PDF_GDRIVE_FOLDER_ID` / `EVIDENCE_ARCHIVE_GDRIVE_FOLDER_ID` / `REPORT_GDRIVE_FOLDER_ID`は実Google Driveへの`--apply`時だけ必須
