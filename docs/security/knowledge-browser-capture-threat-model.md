...
| stale／sensitive local retention            | PWA 60分／10件、extension session storage 10件・論理read TTL 10分、terminal cleanup、expiry read拒否、次のextension実行／session終了時の物理削除、persistent extension storageなし。auth loss後も最後のverified actor keyをmemory-only cleanup capabilityとして保持し、claimed draftのdelete以外には使わない                         |
