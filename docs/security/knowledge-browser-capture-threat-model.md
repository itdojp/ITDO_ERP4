...
| stale／sensitive local retention            | PWA 60分／10件、extension session storage 10件・論理read TTL 10分、expiry read拒否、次のextension実行／session終了時の物理削除、persistent extension storageなし。terminal resultは本文／request keyをcontent-free tombstoneへ先に置換するため、物理削除失敗やreloadでもcontentを復元しない。auth／network loss後も最後のverified actor keyをmemory-only cleanup capabilityとして保持し、tombstone化／delete以外には使わない |
