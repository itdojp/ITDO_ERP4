...
| incognito／InPrivate境界混同                | 生成manifestを`incognito: not_allowed`へ固定し、通常windowとprivate browsing間でsession draft／recent queueを共有しない                                                                                                                                                                                        |
...
| origin confusion／message spoofing          | exact build origin、extension ID、`event.source === window`、event origin、draft ID、schema、one-time nonce、server-confirmed actor由来fingerprintを照合。TTL内のcontent-bearing commandで受理したnonceはevictせず、32件到達後はcontent-free cleanup／delete以外をfail closedにする。fingerprintをserver署名とは扱わない                              |
...
| stale／sensitive local retention            | PWA 60分／10件、extension session storage 10件・論理read TTL 10分、expiry read拒否、次のextension実行／session終了時の物理削除、persistent extension storageなし。terminal result時にhandoff parameterをcurrent addressから外し、tombstone書込み未確認と物理削除待ちを別表示する。tombstone確定後は本文／request keyを保持せず、物理削除失敗やreloadでもcontentを復元しない。auth／network loss後も最後のverified actor keyをmemory-only cleanup capabilityとして保持し、tombstone化／delete以外には使わない |
