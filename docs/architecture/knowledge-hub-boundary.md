...
UI adapterはこの境界を緩和しない。catalog、budget、preview、run responseをallowlistで新しいview modelへ正規化し、unknown field、provider transport設定、raw error、source ID、request keyをcomponent stateへ複製しない。provider disabled時はcatalog以外を取得せず、enabled時は最新ready snapshotだけを最小既定contextとする。preview tokenとrequest keyはcomponent memoryに限定し、commit前または確定済みrunのitem/tab切替で破棄する。dispatch中および送信段階不明の間は同じintentを保護するため親workspaceまでitem/tab切替を抑止し、read responseにはAbortSignalとgeneration guardを適用する。commit応答がnetwork error、invalid response、`execution_failed`、403／404等で不明になった場合はpreview由来のrun IDとrequest keyを保持し、直後の一時404でもdraftを解放しない。同じrun IDのreadだけを再試行可能とし、新しいpreview／request keyによる二重dispatchを防ぐ。preview token不正／期限切れ、stale preview、hard／rate／policy block、idempotency／reservation conflict、明示的なprovider送信前拒否など、server error codeが未送信を保証する場合だけlockを解除する。HTTP statusだけではdispatch確定性を判定しない。確定済みrunを一度取得した後の404はcurrent ACL失効として本文・previewをpurgeする。result unknown／usage unknownは通常成功と視覚・semantic labelの両方で区別し、再送ボタンを提供せず、local reconciliationだけを許可する。grace期間中または新しい保存済み証跡がないreconcileは、成功した状態遷移と区別して「状態変更なし」と表示する。
