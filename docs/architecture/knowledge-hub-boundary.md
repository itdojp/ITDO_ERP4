...
Knowledge runtimeのpreview/commit境界は、routeで受けたselectorをapplication port経由でtyped sourceへ解決し、preview時とdispatch intent確定時の二回、current ACLとexact version/hashを検査する。personal sourceはownerだけ、organization sourceは同一organizationの有効group grantを含む通常read ACLを適用し、detailと結果conversation readでもcurrent source ACLを再評価する。LLM結果conversationのgeneric list/detailはitem relationの有無だけへ依存せず、関連runの全typed context sourceに対するcurrent ACL共通部分をquery内で評価する。LLM結果turnの再選択は拒否してprovenance depth 1を維持する。preview tokenは既存Knowledge signing secretを別domainで使用し、本文やraw IDを含まないactor-bound HMAC envelopeとする。budget previewはread-only、reservationはconfirm後だけであり、再previewでrun IDが変わってもsame-key/same-payload replayはprovider準備・dispatchより前に既存ledgerへ収束する。stub実行は明示的な非production構成だけで利用し、reported usageを伴う結果をmanual conversationのuser/assistant turnへ一度だけ確定する。有効本文にusageが欠落・不正なら本文を同じturnへ固定した`result_ready + held_maximum`とし、通常successにしない。reconcileはprovider enablementとsource disclosureから分離したrun-owner限定のlocal accounting遷移であり、provider requestを作成しない。source ACL失効後もstale reservationをrelease/holdへ収束させた後、responseをcurrent ACLでfail closedにする。OpenAI transportと保存済みoutcomeによるunknown reconciliationは次のadapter段階までruntimeから到達不能とする。
