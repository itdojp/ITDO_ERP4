...
LLM固有auditはtop-level `AuditLog.userId`だけをcanonical actor正本とし、audit writerでcommon auth identifier契約を再検証する。caller supplied principal／delegated actor／scope IDをmetadataへ複製しない。metadataはprovider/model、件数、token/cost、結果code、request correlationと`api|agent`区分だけのallowlistとする。
