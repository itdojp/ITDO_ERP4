...
- PWA share targetは`POST /share-target`のlocal transport adapterであり、domain/application portではない。service workerはstrict multipart normalization後のdraftを専用IndexedDBへ最大10件・60分だけstagingし、opaque 128-bit IDだけで303 landingへ移動する。draft IDとbackend request keyを分離し、request keyはURLへ出さない。各gestureを独立local draftとして扱い、server ledgerが同じrequest keyのreplayを一つのitem/snapshotへ収束させる。既存API offline queueはonline復帰時にmutationを再送する責務を持つため再利用しない。explicit cross-site initiator metadataは拒否する。Chromiumがservice worker Requestへ`Origin`／`Sec-Fetch-Site`を公開しない経路はWeb Share Target互換のlocal stagingとして受理するが、API/cookie/CSRFへ接続せず最大10件・TTL・認証済みpreview/confirmで影響を限定する。未認証時はserver確認前にpayloadをcapture UIへhandoff／renderしない。TTL cleanupはbrowser-local recordを走査・正規化するが、本文をDOM、network、Cache API、logへ渡さない。server確認済みの最初のactorへlocal draftをopaque hashでatomic claimしてから既存capture landingへ一回だけhandoffする。service workerはsessionを解析しないためfirst claim前はbrowser profileがlocal trust boundaryであり、共有端末ではprofile分離または意図したaccountでのfirst loginを要求する。BFF actorはlegacy user fieldではなくcanonical `userAccountId`、header modeはverified user IDを別namespaceでhashする。auth verificationとdraft lifecycleのgenerationを分離し、検証中はhandoffを非表示にしてactor変更結果をremote lifecycleで破棄しない。auth state更新はactor/auth dataを含まない別のBroadcastChannelでtab間通知し、legacy localStorage値が同一でもcanonical actor変更を即時purgeする。previewの前後とcommit／reconcile直前にもserver actorを非永続で再確認し、handoff actor不一致ではlocal pending/API mutationを開始しない。pending IndexedDB transaction中にpurge／unmountが先行した場合は、transactionをatomicに完了してからcaptured actor bindingで`staged`へ補償し、API mutationを開始しない。補償失敗時は`pending`をfail closedで維持する。server側もpreview tokenをcurrent canonical actorへHMAC bindingし、owner-scoped ledger／ACLをcommit・reconcileで再検証するため、frontend再検証後にsession actorが切り替わるnetwork TOCTOUをside effect前に拒否する。local lifecycleは`staged → pending → cleanup_pending`とし、provider dispatch前にbackend正規化済みexact draftとintentを一つのIndexedDB transactionで永続化する。pending CASのwinnerだけがremote lifecycleを通知し、loserはowner commitをabortしない。pending reloadは同じpayloadから新規commitを許さずread-only reconcileだけを復元し、terminal cleanupは本文を先にcontent-free tombstoneへ置換する。extension本文を読む前にsame-originへ固定長のcontent-free dual-slot fenceを予約し、terminal時はどちらか一方のslotを単調に上書きできればreload／late tabをfail closedにする。partial active初期化はterminalと解釈しない。in-flight `get`もdraft dispatch直前にterminalを再検査する。tombstone書込みと物理削除のretry phaseを区別し、tombstone失敗時もidempotent deleteを試し、blocked open後のlate connectionもcloseする。同じtabは一つのlifecycle BroadcastChannel objectを共有して自己通知によるcommit abortを防ぎ、別tabからの`pending|staged`はexact recordを再読込し、terminal通知はcontentをpurgeする。いずれのmessageにも本文、actor、request keyを含めない。認証喪失ではDOM上のhandoffをpurgeする。期限切れpayloadは即時read拒否し、app/service workerの次の実行機会に物理削除する。Cache API、request URL、notification、logへ本文を複製しない。
- Browser Capture extensionもlocal transport adapterであり、Knowledge application portや認証clientではない。Manifest V3の`activeTab` user gestureでmain frameのallowlist fieldだけを抽出し、論理read TTL 10分の`chrome.storage.session` draftへ正規化する。期限後はreadを拒否し、次のextension実行またはbrowser session終了時に物理削除する。ERP4 exact originはbuild artifactのcontent-script matchとして固定し、wildcard host permissionは持たない。extension service workerとERP4 pageのbridgeはextension ID、exact page origin、opaque draft ID、one-time nonce、server-confirmed actorのdomain-separated fingerprintを照合する。このfingerprintはactor切替／claim整合用でありserver署名されたattestationでもsame-origin XSSに対するauthenticationでもない。exact ERP4 originのapplication JavaScript、target環境で検証済みのCSP、XSS防止、locked browser profileを信頼境界とし、そのCSP証跡がないdeploymentではextensionを有効化しない。extensionはERP4 cookie、token、CSRF headerを取得せずAPIを呼ばない。ERP4 landingは認証確認後にdraftを一度だけ既存capture ingressへ渡し、preview／confirm前のmutation、自動retry、local persistent storageを行わない。session終了時のdraft lossは可用性上の制約として受容し、機密本文をpersistent browser storageへ残さないことを優先する。
- capture commitはmanual item createとsnapshot appendを順番に再生せず、owner-scoped request ledger、KnowledgeItem、materialization metadata済みpending KnowledgeSnapshotを同じSerializable transactionへ束縛する。preview tokenはactor／exact payload／scopeに加えてopaque request keyのsecret-derived fingerprintを束縛し、別keyへの再利用を拒否する。organization intentは全selected groupのactive group／membership／canonical user rowを決定順で共有lockする。artifact I/Oはtransaction外で一回だけ行い、その直前、全provider I/O return path、finalization transaction内でpreviewに含まれる全groupのcurrent membership／grantとitem ACLを再検査し、結果不明はpendingのまま保持する。reconcileは署名済みpreview intentとopaque request keyから実ledger captureを解決し、state読取後とprovider I/O後のexact ACLを満たす場合だけ既存owner-scoped artifactをread-only照合してfinalizeする。新しい副作用を生成しない。通常commitではtoken expiryを適用するが、既存pending ledgerのreconcileでは署名、actor、request key、payload bindingを維持したままexpiryだけを回復の拒否理由にせず、失効済みACLでは必ず404とする。responseは実capture IDとrequest preview IDを別fieldで束縛し、offline replayまたはHTTP応答喪失後に新preview IDが発行されてもitem増殖と誤capture照合を防ぐ。Serializable retry枯渇はsanitized conflictとして返し、raw SQL／stackを外部化しない。
...
Knowledge Hubの外部AIは、Chat summary固有関数ではなく共有provider-neutral portへ依存する。共有adapterはprovider I/Oなしの同期`bind`、非同期`prepare`、単一使用`dispatch`を分け、prepareでserialization、URL/DNS/non-public-address検査、DNS pin、body変換を終える。pre-dispatch検証timeoutとprovider network timeoutは分離し、後者はdispatch開始時から計測する。Knowledge側は未対応surrogateを拒否したversion付きcanonical provider body bytesだけでなく、canonical final endpointとhost allowlist／HTTP・private-IP許可／timeout／response上限／response policyを含むtransport contractをopaque SHA-256へ束縛し、これをimmutable provider request hashとして予約する。raw endpoint、allowlist、credentialはrun、audit、logへ保存しない。prepared fingerprintと保存済みhashを照合し、runのdispatch intent／mandatory auditをcommitしてから同じprepared requestをdispatchする。selected contextはsourceごとに独立したprovider messageへserializeし、source本文内のdelimiter-like文字列が別sourceまたはuser promptの境界へ化けないようにする。contextを持たないChat requestは従来どおりsystem/user二messageのpayloadを維持する。prepare後のcaller object変更でbody、model、policyは変化しない。これにより確定的pre-dispatch failureだけをreleaseし、intent確定後のtimeout/socket failureをunknown/held maximumとして扱える。adapterはtransport安全性だけを所有し、Knowledge側がselected context、exact source version、preview/confirm、ACL、budget reservation、idempotency、監査を所有する。4xx/5xxのprovider本文はpromptやsecretを反射し得るため監査可能なerrorへ連結せず破棄し、HTTP statusと固定failure codeだけを正規化する。成功header後のbody timeoutは結果不明、DNS／scheme／host／non-public-address guard failureはtypedな未dispatchとして区別し、空の成功本文をKnowledgeの通常成功にしない。`allowPrivateIp=false`はIANA IPv4/IPv6 Special-Purpose Address Registry（実装基準日2025-10-09）の非グローバル範囲とmapped/link-local/unique-local/multicast等をliteral/DNS双方で拒否し、registry更新時にCIDR一覧とnegative fixtureを同時更新する。private IPを明示許可する非production構成でもDNSを接続前に解決・pinし、Node lookupのfamily/all契約と解決失敗のcertaintyを維持する。providerの成功本文とusageは別々に正規化し、有効な本文にusage欠落または不正usageが付随する場合は本文を失わず明示的な`missing|invalid`状態を返す。Knowledge側はこれを通常成功にせずmaximum reservationを保持する。provider固有の非課金保証がない4xxもreleaseせずmaximumを保持する。Chatの既存prompt、`CHAT_EXTERNAL_LLM_*`、user/room rateと空応答fallbackは独立した互換wrapperに残す。

host allowlistはraw入力をUnicode case fold前にprintable ASCIIとして検査し、IPv6 literalはURL側の角括弧を除いたcanonical表現へ統一する。provider actual usageが予約済みconservative input estimateまたは要求したmax outputを超える場合は`usage_invalid`として通常settlementせず、maximum reservationを保持する。

read-only budget previewもcurrent active policy IDだけを集計対象にしない。personalはuser subject、organizationはuser／organization両subjectについて、同じcurrent monthly windowに属するinactive versionのperiod counterとrolling 60-minute reservationを同一`RepeatableRead` snapshotでsubject単位に合算する。表示用previewのsoft／hard／rate判定はauthoritative reservationと同じ定義を使用し、policy version更新で見かけ上の利用量がresetされないようにする。

予算正本はPostgreSQLのversioned policy、月次period、run、request ledger、reservationである。personalはuser policy、organizationはuser/org policyを同一currencyで決定順lockし、hard limitを`settled + active + held + new maximum`で評価する。actor／subject／organization／createdByは共通auth identifier契約でapplicationとDBの両方がcanonical性を検証し、policy timezoneは前後空白なしのIANA名だけを許可する。active policyの選択とrow lockは同一statementで行い、同時version切替はSerializable bounded retryでactive versionを再解決する。active policy lock下でcurrent periodを発見して不足分だけを作成し、この段階では既存periodをlockしない。その後、全subjectのcurrent periodとrolling 60-minute窓に重なるhistorical periodのunionを単一queryで取得し、settlementと同じglobal period ID順で一度だけlockしてからsubject別usageを読む。lock後にcurrent periodの境界／timezone／currencyを再検査する。これによりcurrentとhistoricalでuser/orgの任意UUID順が逆転してもreservationとsettlementのperiod lock cycleを作らない。現在windowと重なるinactive versionのperiodが異なるcurrency/timezoneを持つ場合は、旧月境界を現在policyで再解釈せずfail closedとする。rolling 60-minute窓だけに残る終了済みperiodはrate countへ含めるがdrift比較から除外し、current period不整合／無効timezoneはcurrent period作成を含むtransactionをrollbackした後にtyped budget-blocked auditと`policy_mismatch`へ正規化する。period INSERT triggerはpolicyのtimezone/currencyと、当該IANA timezoneでの月初から翌月初までのcanonical UTC境界を再計算し、重複・重なりにつながる非canonical windowを拒否する。月初ローカル時刻がoffset後退で二重化する場合はPostgreSQLとapplicationの双方が後側UTC instantをcanonicalとし、別instantを拒否する。policy metadataとperiod境界はversion内でimmutableなため、counter-only UPDATE triggerはpolicyを再lockせず、reservationのpolicy→periodとsettlementのperiod更新の間に逆順lockを作らない。reservation INSERT triggerが初期run、subject、period、currency、amount、timestampを検証し、callerが指定できないPostgreSQL UTC `accountedAt`を付与する。accountedAtを含むcurrent periodだけを許可し、runに必要なuser／organization全subjectのactive policyを共通順序で、current-windowとrolling 60-minute窓のperiod unionをglobal ID順でlockしてinactive versionおよび月初直後の前月periodを含むhard limit、soft warning、rolling rateをDBでも再検査してからperiod counterを同じtransactionで更新する。dispatch後の追加予約、同一subject二重予約、過去period／backdated provenance timestampによるaccounting迂回を防ぐ。organization runのuser/org reservationを逆順で挿入するdirect writerも、最初のrowで全required subjectをlockするためcross-subject deadlockを作らない。applicationが選択したperiodとlock後のDB accounting instantが月次境界でずれた場合は、run／period mutationをrollbackし、named DB diagnosticを本文なしのmandatory auditとtyped `policy_mismatch`へ正規化して再previewを要求する。policyのIANA timezoneとrequest非依存のserver clockから月次UTC境界およびrolling rate時刻を算出し、process timezoneやclient timestampを使用しない。reservation/dispatch/settlement/reconcileも同じtrusted clock境界を使用する。execution stateとsettlement stateを分離し、provider結果またはusageが不明なら通常successにせずmaximum reservationを保持する。

hard-limit残高から除外される累積release counterは、月内の反復reserve/releaseでもBIGINT overflowしない`NUMERIC(38,0)`のinteger micro-unitとする。reservation transition triggerが検証済み状態遷移と同じtransactionでperiod counterをO(1)差分更新し、period counterへの直接更新は拒否する。通常mutationごとのledger全件再集計は行わず、全件一致の監査はread-only `erp4_knowledge_llm_assert_period_accounting(period_id)`をoperatorが明示実行するoffline reconciliationへ分離する。

provider call中はDB transactionを開かない。dispatch前にrun/reservation/audit、catalog単価snapshot、ordered selected-context rowを同一transactionで確定し、provider/model/version/単価/currency/maximum costは有効catalogからuse caseが解決してbudget port入力を再構築する。最大reservationのinput estimateとprovider request hashは、system prompt、独立上限を適用したraw user prompt、typed source identity/version/hashとexact representationを一体化したordered context、source framing、固定provider message framingからuse case内で導出し、callerはfingerprintや値を独立指定して減らせない。run INSERT時はDB CHECKも凍結input/output token上限と単価から各項切り上げ式でmaximum reservationを再計算し、DB-side writerの過少予約を拒否する。request ledger hashはcanonical provider requestに加え、effective input estimate、凍結input/output単価、currency、maximum reservationへ束縛し、同じrequest keyでも経済条件が変わる再送をconflictにする。PR Aのbudget foundationはcaller supplied preview hashを確認済みの根拠として受け取らず、実prompt/context/model/scopeからrequest ledger hashを再計算する。後続PR Bのpreview verifierは同じcanonical provider request hashと経済条件へ署名tokenを束縛し、照合後にだけreserveへ進む。soft-limit warningはrunへ固定してidempotent reuseでも保持する。provider callは自動retry・fallbackなしで一回だけ行う。normalized outcomeは本文をcaptureした未finalized rowとして保存し、DBがdomain-separated SHA-256を再計算してcontent hashを検証した後にだけ本文消去を伴うfinalizeを許可する。outcome finalizeとsettlement/reconcileはoutcome→run→reservation→periodの決定順lockを共有する。settlementはassistant/AI turnへのhash束縛、snapshot単価とusageからのactual cost再計算、typed完了／失敗／結果不明／usage不明auditを同じtransactionで確定し、run transition triggerも凍結単価とusageから同じinteger cost式を再計算して直接SQLによる過少計上を拒否する。dispatch timestampは最初の`reserved -> dispatched`でだけ設定でき、それ以降の中間・terminal状態で変更できない。reservation rowの削除はtriggerで拒否し、period counterとrolling rate accountingを保持する。`result_ready + held_maximum`はfinalized `usage_unknown` outcomeとassistant/AI turnが一致する場合だけ許可し、finalized outcomeの直接INSERTはDBで拒否する。後日usageが確認できた場合はprovider outcomeを改変せず、immutable operator billing evidenceを追加し、run ownerとは別にauthenticated operatorのcanonical user IDをevidence createdByとmandatory auditへ保存する。audit writerとDB identity CHECKはcommon auth identifier契約を再検証し、raw Unicode別表現によるself-settlement迂回を拒否する。operator interventionは専用actorRole/reasonCode/allowlisted分類で識別する。snapshot単価でactual costを再計算し、同じtransactionでheldからactualへ一方向精算する。同一evidenceの再照合は同じoperatorだけidempotent、異なるoperator/evidenceはconflict、evidence更新・削除は禁止とする。このevidenceはtrusted billing operationsが別途確認したusage assertionを不変記録する内部境界であり、application／DB自身は外部billing artifactの真正性を検証しない。artifact verifierとoperator authorizationが同時に実装されるまで公開routeへ接続せず、安全なlookup／検証済みartifactがなければ`held_maximum`を維持する。timeout、connection outcome unknown、finalization failureは`failed`ではなくreconcile可能な`result_unknown + held_maximum`へ限定する。`result_unknown + held_maximum`の全許可failure codeは、同一runのvalid/finalized outcomeを後から安全に取得できた場合だけreconcile可能とする。安全なprovider outcome lookupがない場合は再dispatchせず、unknown/held maximumを維持する。 reconcileのstale判定graceはprovider network timeout上限120秒より長い150秒に固定し、in-flight dispatchをunknownへ早期遷移させて後着結果のcaptureを拒否する競合を防ぐ。

LLM固有auditはtop-level `AuditLog.userId`だけをcanonical actor正本とし、audit writerでcommon auth identifier契約を再検証する。provider model identityはapplicationとDBの専用validatorで前後のECMAScript TrimString文字、C0/C1 control、Unicode 15.0 `General_Category=Format (Cf)`、ill-formed surrogateを拒否し、1〜200 Unicode code pointsへ固定する。caller supplied principal／delegated actor／scope IDをmetadataへ複製しない。metadataはprovider/model、件数、token/cost、結果code、request correlationと`api|agent`区分だけのallowlistとする。

selected contextはtyped FKとexact version/hashを持つimmutable rowで表現し、自由な`sourceType + sourceId`を正本にしない。dispatch時にDBはFK先のversion/hash、domain-separated representation hash、`UTF-8 bytes * 2 + 16 framing tokens`、種別別・合計・関連item・provenance depth上限を再検証し、source IDを含むorder-sensitive opaque fingerprintへ束縛する。runのconversation referenceはresult_ready遷移時だけ設定でき、reserved/dispatched/failed/result_unknownではnullを維持する。assistant resultは同じownerの`KnowledgeConversation`と、そのconversationに属するassistant turnの複合FKへ結び付ける。provider outcomeは最大256 KiBのnormalized contentだけをfinalizationまで一時保持でき、finalize時に本文をconversation turnへ移してoutcome rowから消去する。利用者promptはprovider dispatchより前にrunへ一対一の`KnowledgeLlmPromptSnapshot`として保存し、domain-separated hash、16 KiB上限、canonical actorをDBでも検証する。prompt snapshotは更新・削除を禁止し、結果conversationのuser turnへ移した後、またはprovider resultが存在しない確定terminal状態へ進めた後にだけ本文を一方向に消去する。provider outcomeとprompt snapshotのdeferred reverse consistency triggerは、本文だけを消去してrun/conversation/settlementを未確定のままcommitすることを拒否する。

Knowledge runtimeのpreview/commit境界は、routeで受けたselectorをapplication port経由でtyped sourceへ解決し、preview時とdispatch intent確定時の二回、current ACLとexact version/hashを検査する。synthesisはcurrent versionへの暗黙追従を行わず、選択したexact履歴versionを許可する一方、non-ownerに対してはそのversionの全direct provenance sourceを候補、preview、commit、detail、結果conversationの全経路でcurrent ACL再評価する。候補取得と複数sourceのread再検査は同一Repeatable Read snapshot、dispatch intent確定は既存Serializable transactionで行う。personal sourceはownerだけ、organization sourceは同一organizationの有効group grantを含む通常read ACLを適用し、detailと結果conversation readでもcurrent source ACLを再評価する。LLM結果conversationのgeneric list/detailはitem relationの有無だけへ依存せず、関連runの全typed context sourceに対するcurrent ACL共通部分をquery内で評価する。LLM結果turnの再選択は拒否してprovenance depth 1を維持する。source pickerのpagination cursorは既存signing secretからdomain-separated keyを導出したAES-256-GCMで認証暗号化し、request URL/log上のtokenからsource internal IDを復元できないようにする。preview tokenは既存Knowledge signing secretを別domainで使用し、本文やraw IDを含まないactor-bound HMAC envelopeとする。request-specific budget previewはcatalog currencyをbudget portへ渡し、policy currencyとの不一致をreservation前にfail closedとする。budget previewはread-only、reservationはconfirm後だけであり、再previewでrun IDが変わってもsame-key/same-payload replayはprovider準備・dispatchより前に既存ledgerへ収束する。stub実行は`NODE_ENV=test|development`の明示構成だけで利用し、staging、production、環境未指定ではfail closedにする。reported usageを伴うstub結果はmanual conversationのuser/assistant turnへ一度だけ確定する。有効本文にusageが欠落・不正なら本文を同じturnへ固定した`result_ready + held_maximum`とし、通常successにしない。OpenAI-compatible実行も明示enable・allowlisted model・安全なtransport条件を満たす場合だけ同じrun境界へ接続し、自動retry／fallbackを行わない。reconcileはprovider enablementとsource disclosureから分離したrun-owner限定のlocal accounting遷移であり、provider requestを作成せず、DBへ既に保存されたexact outcomeだけを確定する。source ACL失効後もstale reservationをrelease/holdへ収束させた後、responseをcurrent source ACLでfail closedにする。

source internal IDはReact state、memoized request、DOMへ保持せず、module-private registryでopaque candidate keyへ対応付け、preview／execute直前にだけselectorへ解決する。candidate page順に依存せず、ready snapshotの最大versionを最小既定contextとして選ぶ。

UI adapterはこの境界を緩和しない。catalog、budget、preview、run responseをallowlistで新しいview modelへ正規化し、unknown field、provider transport設定、raw error、source ID、request keyをcomponent stateへ複製しない。provider disabled時はcatalog以外を取得せず、enabled時は最新ready snapshotだけを最小既定contextとする。preview tokenとrequest keyはcomponent memoryに限定し、commit前またはterminal runのitem/tab切替で破棄する。dispatch中、送信段階不明、またはrunが`reserved|dispatched`の間は同じintentを保護するため親workspaceまでitem/tab切替を抑止し、read responseにはAbortSignalとgeneration guardを適用する。commit応答がnetwork error、invalid response、`execution_failed`、403／404等で不明になった場合はpreview由来のrun IDとrequest keyを保持し、直後の一時404や非terminal run readでもdraftを解放しない。同じrun IDのreadだけを再試行可能とし、`result_ready|failed|result_unknown`到達時だけlockを解除して、新しいpreview／request keyによる二重dispatchを防ぐ。preview token不正／期限切れ、stale preview、hard／rate／policy block、idempotency／reservation conflict、明示的なprovider送信前拒否など、server error codeが未送信を保証する場合だけlockを解除する。HTTP statusだけではdispatch確定性を判定しない。terminal runを一度取得した後の404はcurrent ACL失効として本文・previewをpurgeする。result unknown／usage unknownは通常成功と視覚・semantic labelの両方で区別し、再送ボタンを提供せず、local reconciliationだけを許可する。grace期間中または新しい保存済み証跡がないreconcileは、成功した状態遷移と区別して「状態変更なし」と表示する。
