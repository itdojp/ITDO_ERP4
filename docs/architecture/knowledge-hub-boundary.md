...
- capture commitはmanual item createとsnapshot appendを順番に再生せず、owner-scoped request ledger、KnowledgeItem、materialization metadata済みpending KnowledgeSnapshotを同じSerializable transactionへ束縛する。artifact I/Oはtransaction外で一回だけ行い、その直前とfinalization transaction内でcurrent item／ACLを再検査し、結果不明はpendingのまま保持する。reconcileは署名済みpreview intentとopaque request keyから実ledger captureを解決し、provider I/O前後のcurrent ACLを満たす場合だけ既存owner-scoped artifactをread-only照合してfinalizeする。新しい副作用を生成しない。responseは実capture IDとrequest preview IDを別fieldで束縛し、offline replayまたはHTTP応答喪失後に新preview IDが発行されてもitem増殖と誤capture照合を防ぐ。
