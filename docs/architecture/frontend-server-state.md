...
- message load は request sequence と target room を照合し、遅延した旧 room response が現在 room の messages / unread / loading state を上書きしない。unread queryもroom IDだけでなくrequest sequenceへbindし、同じroomに対する古いresponseが新しい既読状態を巻き戻さない。
...
- non-idempotentなroot message / root確認依頼 / reply POSTは、明示的な4xx rejectionの場合だけdraftを保持して修正・再送を許可する。transport failure、5xx、不整合2xxで結果を確認できない場合はcomposerをlockし、同じ画面からの再送を禁止する。root POST lifecycleはApp sessionが所有し、`RoomChat` sectionのunmount/remountやroom変更だけで解除しない。root POST中と結果不明後はroom selector、room deep link、room作成、root timeline操作と新規thread openを無効化する。requestは開始roomへ固定し、異なるroomで完了した非同期結果を現在stateへ適用しない。POST待機中にsectionがunmountされた場合はlifecycle結果だけをAppへ確定し、添付upload、timeline refresh、read mutation等の後続副作用を開始しない。結果不明時はpage reloadを再開境界とする。mutation成功後はGETで同一reply IDを確認できた本文だけを表示し、後続page境界やrefresh failureで確認できないPOST response本文はstateへ合成しない。「再送せず再読み込み」を案内して二重投稿と削除済み本文の再表示を防ぐ。
...
- room selector、room event、root deep linkの全room変更経路で旧thread stateとfocus/deep-link/highlight stateを破棄し、取得失敗時に前roomのthread本文を残さない。thread取得・thread mutation・reply/ACK reply POST・root POST・root timeline上のreaction/ACK/revoke/cancelが403/404になった場合は、mutation開始時のroom identityが現在roomと一致することを先に確認する。一致する場合だけthreadとglobal searchを消去し、current roomを無filterで再取得してread ACLを再検証する。再取得成功時だけ最新timelineを保持し、失敗時はroom-bound timeline、thread、unread、filter、global searchをpurgeする。遅延した旧roomのaccess callbackは現在room stateを変更しない。対象replyだけの同時削除で再取得が成功した場合は、最新threadを保持して対象単位の競合とroom/root access lossを区別する。
...
## 2026-07-13 の更新: AdminSettings の resource hook / form-state 境界

Issue #1924 では React Query 依存を追加せず、既存 `api` wrapper を使った resource-specific hook 境界を固定した。単一の巨大 generic hook は作らず、設定カテゴリごとの query / mutation / form / validation / reset / feedback を個別hookで扱う。

| boundary                     | file                                                       | responsibility                                                                                           |
| ---------------------------- | ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| common helper                | `admin-settings/adminSettingsResourceUtils.ts`             | JSON parse error feedback、nullable text normalization、message/error sink 型                            |
| policy resources             | `admin-settings/useAdminSettingsPolicyResources.ts`        | approval rules、ActionPolicy、chat ack templates、audit history、form parse/serialize、save/reset/toggle |
| template resources           | `admin-settings/useAdminSettingsTemplates.ts`              | template settings、PDF template query、kind別auto-select、save/default/reset                             |
| report resources             | `admin-settings/useAdminSettingsReports.ts`                | report subscriptions、deliveries、dry-run、run-all、form parse/serialize                                 |
| integration resources        | `admin-settings/useAdminSettingsIntegrations.ts`           | integration settings、run history、metrics、run trigger、form parse/serialize                            |
| integration export jobs      | `admin-settings/useAdminSettingsIntegrationExportJobs.ts`  | export job filters、load、redispatch idempotency                                                         |
| reconciliation resources     | `admin-settings/useAdminSettingsReconciliation.ts`         | period key、summary/details query、stale response guard                                                  |
| accounting mapping resources | `admin-settings/useAdminSettingsAccountingMappingRules.ts` | rule filters、form validation/save/reset、reapply、duplicate-submit guard                                |
| local UI state               | `AdminSettings.tsx`                                        | page layout、alert wizard/draft autosave、system_admin visibility、hook wiring、summary metrics          |

### AdminSettings の query / mutation 方針

- 初期表示では `AlertSettings`、policy resources、template/PDF、integration/report/accounting resources を明示的に読み込む。
- save成功後は該当resource hook内で再取得し、form reset と success feedback を同じ境界で実行する。
- JSON parse / CSV parse / nullable field normalization は hook/model 側で行い、top-level component からAPI payload詳細を取り除く。
- reconciliation details は request id を用いて、period変更後の旧responseが現在の details / loading state を上書きしない。
- mutation submit は in-flight ref で二重送信を抑止する。
- existing child cards that already own resource state（chat/scim/group/rate/worklog/auth migration）はこのPRでは保持し、generic hook化しない。

### 行数とテスト

- `AdminSettings.tsx`: 2439 行 -> 929 行。
- 追加/拡張テスト:
  - `adminSettingsHooks.test.tsx`
  - `AdminSettings.test.tsx`（既存containerテスト継続）
  - `GroupManagementCard.test.tsx` と `RoomChat.test.tsx` の全体suite安定化
- 詳細な検証証跡: `docs/test-results/2026-07-13-issue1924-adminsettings-resource-hooks.md`
