...
| 通知抑制（チャット） | `muteAllUntil` / `notifyMentions` / `notifyAllPosts` が通知作成を抑止 | backend unit（`packages/backend/test/chatMentionNotifications.test.js` / `packages/backend/test/chatAckReminders.test.js`） + E2E（`packages/frontend/e2e/backend-notification-suppression.spec.ts` ※ `approval_pending` バイパス含む） | A | `chat_ack_escalation` など残りバイパス種別のE2Eを追加 |
