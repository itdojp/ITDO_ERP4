...
- workflow subset coverage 計測と json-summary 閾値チェック: `npm run coverage:workflow:check`
  - branch protection の job 名を増やさず、既存必須 `CI / backend` の中で失敗させる
  - 閾値判定は `packages/backend/coverage-thresholds.json` の `workflow.files` に列挙した Workflow route / service / application orchestration を `coverage-summary.json` から再集計する
...
### CI / backend workflow coverage

- workflow 関連 subset の coverage 計測と json-summary 閾値チェック: `npm run coverage:workflow:check --prefix packages/backend`
  - `coverage:workflow:check` は内部で `coverage:workflow` を呼び出す
  - 対象は `packages/backend/coverage-thresholds.json` の `workflow.files` を正本とする
  - scope は `bounded-context-registry.cjs` の `workflow` context（ActionPolicy / ApprovalRules / PeriodLocks route と関連 service）に、`src/application/workflow/**` と escalation service（`src/services/approvalEscalation.ts`）を加えたもの
  - `packages/backend/test/coverageThresholds.test.js` は、Workflow context registry と workflow coverage scope の差分、stale entry、閾値の意図しない低下を検出する
  - workflow 以外の backend ファイル追加は `workflow` scope の分母に含めない
- 初期閾値（2026-07-14、#1964 baseline）:
  - statements: 70.7%
  - lines: 70.7%
  - branches: 70.5%
  - functions: 84.8%
- 目的: ActionPolicy、Approval、Evidence gate、Period lock、Reassignment の主要状態遷移と副作用順序に関わる Workflow 境界の coverage 低下を、既存必須 `CI / backend` job で検知する
...
| workflow     | `CI / backend`       | `packages/backend/coverage/workflow/coverage-summary.json`     | `packages/backend/coverage-thresholds.json`  | statements/lines 70.7%、branches 70.5%、functions 84.8%        |
| ui-core      | `CI / frontend`      | `packages/frontend/coverage/ui-core/coverage-summary.json`     | `packages/frontend/coverage-thresholds.json` | statements 68.0%、lines 70.5%、branches 61.0%、functions 67.0% |
...
workflow scope の初期対象ファイルは `packages/backend/coverage-thresholds.json` の `workflow.files` を正とし、`src/routes/actionPolicies.ts`、`src/routes/approvalRules.ts`、`src/routes/periodLocks.ts`、Workflow context service、`src/application/workflow/**`、`src/services/approvalEscalation.ts` を対象にする。新しい Workflow route / service / application orchestration file を追加した場合、`coverageThresholds.test.js` が `workflow.files` の更新漏れを検知する。設定済みファイルの削除・リネームは coverage checker の stale file 検査で失敗する。
