...
## UI変更PRにおける証跡更新の判断
- 次のいずれかに該当する場合、証跡更新を必須とする
  - 画面の見た目/導線/文言を変更した
  - 新規UI（新画面・新ダイアログ・新タブ）を追加した
  - マニュアルに掲載済みの主要導線（Dashboard/日報/工数/経費/休暇/承認/仕入発注/チャット/監査）を変更した
- 証跡更新時は、PR本文に次を記載する
  - 実行コマンド（例: `./scripts/e2e-ui-evidence.sh`）
  - 追加/更新した記録（`docs/test-results/YYYY-MM-DD-*.md`）
  - 参照ディレクトリ（`docs/test-results/YYYY-MM-DD-*/`）
- UI変更が軽微で証跡更新が不要と判断した場合も、PR本文に「更新不要の理由」を明記する
