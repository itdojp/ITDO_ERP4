...
- [ ] #339 GRC/監査 PoC（監査ログ/アクセス棚卸し）
  - [ ] 監査ログの検索/CSV出力API（期間/ユーザ/アクション/対象）
  - [ ] 監査ログ出力時の監査ログ記録
  - [ ] アクセス棚卸しスナップショット出力（ユーザ/グループ/状態）
  - [ ] docs/requirements/grc-audit.md に手順/仕様を追記
- [ ] #340 AI/分析 PoC（インサイト生成/表示）
  - [ ] ルールベースのインサイト生成（予算超過/残業増/承認遅延）
  - [ ] インサイトAPI（期間/プロジェクト指定）
  - [ ] 管理ダッシュボードの簡易表示（一覧/カード）
  - [ ] 閲覧操作の監査ログ記録
  - [ ] docs/requirements/ai-analytics.md に手順/仕様を追記
