...
- [ ] #347 PoC評価（監査ログ/アクセス棚卸し/インサイト）
  - [ ] 監査ログエクスポートの動作確認（検索条件/CSV/JSON/監査ログ記録）
  - [ ] アクセス棚卸しスナップショットの確認（出力項目/権限/内容）
  - [ ] インサイト生成の確認（Alert集計/件数/サンプル対象/権限）
  - [ ] 確認結果を記録し、課題をIssue化
- [ ] #348 本実装計画（GRC/AI 強化）
  - [ ] 監査対象イベントの拡張方針を整理
  - [ ] 監査ログの外部保全方針（WORM/S3 Object Lock 等）を整理
  - [ ] インサイトの追加指標/根拠表示の仕様を整理
  - [ ] スケール/性能/保持方針を整理
- [ ] #349 運用設計（監査/アクセスレビュー/インサイト）
