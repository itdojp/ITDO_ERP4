...
- [ ] 未決定/要設計の仕様確定（DM/既読/メンション/検索/通知/AI/外部ユーザ） (#435)
  - [ ] DMの扱い（管理者設定で禁止、ownerの扱い、external_chat禁止）を確定
  - [ ] 既読表示モード（一覧/人数のみ/非表示）のデータモデルとUXを確定
  - [ ] メンション（ユーザ/グループ/全員）の仕様（通知/権限制御）を確定
  - [ ] Markdown方言とサニタイズ方針を確定
  - [ ] 添付の保存先（Google Drive候補）の方式決定
  - [ ] 検索（チャットのみ/ERP横断）スコープと権限制御を確定
  - [ ] 通知チャネル/条件/既定値/ミュート/頻度制御を確定
  - [ ] AI機能（要約/アクション抽出/FAQ/検索支援）の範囲と監査を確定
- [ ] ガバナンスと監査break-glassの要件確定/実装計画 (#434)
- [ ] ルーム機能（プロジェクト/部門/全社/私的グループ/DM）を実装
- [ ] ルーム一覧/参加/作成/設定UI（公式/私的/外部ユーザ許可）を実装
- [ ] 既読/未読（未読件数/既読更新/表示モード）を実装
- [ ] メンション検出/通知/入力支援を実装
- [ ] Markdownレンダリング/プレビュー/サニタイズを実装
- [ ] 添付（保存/取得/権限制御/上限/ウイルス対策）を実装
- [ ] 検索（チャット内/ERP横断）を実装
- [ ] 通知（アプリ内/メール/Push/外部連携）を実装
- [ ] AI支援（要約/アクション抽出/FAQ/検索支援）を実装
