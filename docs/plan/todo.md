...
- [ ] HR/CRM 連携の実装を本番化（ジョブ/差分同期/エラー処理） (#317)
  - [ ] 連携対象のテーブル/フィールドのマッピング確定（docs更新）
  - [ ] 差分同期のキー（updatedAt/externalId）と衝突解決ルールを定義
  - [ ] integration_runs に retry/backoff を追加し失敗時アラートを実装
  - [ ] 手動実行/スケジュール実行の管理画面を整備（最小UI）
- [ ] モバイル/PWA の実運用化 (#318)
  - [ ] オフラインキュー（TimeEntry/Expense/日報）の送信順序とリトライ設計
  - [ ] 競合時の挙動（上書き/差分提示）の最小UXを決定
  - [ ] Push 通知の配信条件/同意UXを整備
- [ ] 自動化ワークフロー/レポート拡張の本実装 (#319)
  - [ ] 定期レポートの生成（CSV/PDF）を本実装に置換
  - [ ] 送信履歴と配信失敗のリトライ/停止条件を確定
  - [ ] 配信先（メール/ダッシュボード）の優先度/無効条件を決定
- [ ] ロードマップの更新（Phase 1完了の反映とPhase 2の期日整理） (#320)
