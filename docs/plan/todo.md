...
### 本番化・運用整備（残タスク）
- [ ] 性能チューニングの実測（主要クエリのEXPLAIN/統計収集） (#154)
  - [ ] 承認一覧/アラート/工数集計のクエリ計画を docs に記録
  - [ ] index/partition/summary の判断メモを追記
- [ ] 運用監視の最小設計 (#155)
  - [x] バッチ失敗/アラート遅延の監視観点を整理
  - [x] バックアップ/リストア手順の草案作成

## Phase 2（拡張・2026H1）
- [ ] HR/CRM 連携拡張の要件整理 (#156)
  - [ ] HR: wellbeing/ID 連携のデータ範囲/匿名化ポリシー
  - [ ] CRM: 顧客/業者/連絡先の同期範囲とマッピング
  - [ ] 連携方式/頻度/エラー処理の整理
- [ ] モバイル対応 PoC 設計 (#157)
  - [ ] PWA/Capacitor の方針決定
  - [ ] オフライン下書き（IndexedDB/SQLite）のフロー設計
  - [ ] Push 通知の MVP 仕様（APNS/FCM）整理
- [ ] 自動化ワークフロー/レポート拡張の設計 (#158)
  - [ ] 定期レポート生成・配信の要件整理
  - [ ] 承認フローの自動分岐/条件追加の設計
  - [ ] レポートテンプレの拡張方針
