...
## 次の実装（運用・UI整備）
- [ ] 顧客/業者マスタのUI（一覧/検索/登録/編集） (#188)
- [ ] 取引先連絡先（Contact）のCRUD（API + UI） (#189)
- [ ] Project の customerId 選択とバリデーション（API + UI） (#190)
- [ ] projectId 手入力の一覧選択化（工数/経費/請求/レポート/チャット） (#191)
- [ ] Vendor関連画面の整備（PO/仕入見積/仕入請求の一覧・詳細で名称表示） (#192)
- [ ] QA手順の更新（マスタ/連絡先/プロジェクト連携） (#193)
