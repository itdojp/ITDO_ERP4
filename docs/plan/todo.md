...
- [ ] 未決定/要設計の仕様確定（DM/既読/メンション/検索/通知/AI/外部ユーザ） (#435)
  ...
  - [ ] 未読/既読（自分のみ可視化、他者の既読表示なし）のデータモデルとUXを確定
  - [ ] 指定対象者の「OK/確認」状況を追える確認メッセージ（特別メッセージ）を設計
  - [x] メンション（ユーザ/グループ/全員、補完選択）仕様（@allは投稿前確認 + 投稿回数制限）を確定
  - [ ] Markdown方言とサニタイズ方針を確定
  - [ ] 添付の保存先（Google Drive: システムユーザ専用領域案）の方式決定
...
- [ ] 自分の未読/既読（未読件数/既読更新/未読ハイライト）を実装
- [ ] 確認メッセージ（指定対象者のOK/確認）を実装
- [ ] メンション検出/通知/入力支援を実装
- [ ] @all 投稿前確認 + 投稿回数制限を実装
