...
## 次アクション（Phase 1 本番化）
- [ ] PDF/メール本番運用の設定/QA/手順整備 (#244)
  - [ ] 環境変数/必要ファイルと送信ログ/再送の確認手順を整理
- [ ] IDaaS/SCIM 連携の運用フロー/テスト整備 (#245)
  - [ ] 同期フロー/代表ケース/監査ログ確認の項目化
- [ ] 性能チューニング: ベンチマーク/目標SLA/計測手順 (#246)
  - [ ] 代表クエリのSLAと計測手順の叩き台
- [ ] モバイルPoC: PWAスケルトンとオフライン下書き最小実装 (#247)
  - [ ] manifest/service worker と下書き保存の最小実装
