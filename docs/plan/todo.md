...
## 次アクション（意思決定・運用確定）
- [ ] #365 ID管理の未決定事項整理（IdP/SCIM/メール変更）
- [ ] #366 アクセス制御の未決定事項整理（ロール/ABAC/PBAC/RLS）
- [ ] #367 DBマイグレーション運用の未決定事項（本番権限/移行統合）
- [ ] #368 付け替え運用の最終化（締め期間/理由コード/取消手順）
- [ ] #369 バックアップ/リストア本番値の確定（保持/暗号化/S3/PDF）
## 次アクション（Phase 2 実装開始）
