...
## Production Readiness Gate（#1875 / 2026-07）

状態区分:

- **実装完了**: repo上のコード・文書・自動テストが存在する。
- **repo-side readiness完了**: 現在のcommitでCI/runnerにより再現可能に検証できる。
- **対象環境での運用完了**: 実環境設定値、rollback、backup/restore、外部製品現物を使った確認が完了している。
...
- [ ] #544 S3バックアップ設定値確定と実backup → upload → download → restore検証
- [ ] #1432 給料らくだ・経理上手くんαの現物CSVテンプレート／サンプル回収
- [ ] #1430 外部連携仕様全体の正本を継続管理（#1432未完了項目は推測で確定しない）
...
- [ ] 未実装項目の解消（優先度D: 実配信/外部連携）
  ...
  - [x] #560 BE: 添付AVスキャンのPoC実装（ClamAV/clamd + docs + smoke）
  ...
  - [x] #886 AVスキャン本番有効化方針（`disabled` 継続 or `clamav`）の決定（2026-02-11 close）
  - [x] #886 定義更新/監視/障害時の運用方針の最終決定（`docs/requirements/chat-attachments-antivirus.md`、2026-02-11 close）
...
- [x] データ移行マッピング初版（PO im__/acs__ → 新スキーマ、キー/シーケンス整合）
