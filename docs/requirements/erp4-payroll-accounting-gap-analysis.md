...
| 締め状態/締め基準日                       | `AttendanceClosingPeriod.status/closedAt`                              | 条件付き | 同一 `periodKey` の global `PeriodLock` を締め前提とする |
| 出勤日数/残業/遅刻早退/有給消化の確定集計 | 出勤日数、総残業、法定内/法定外/休日労働、paid-unpaid leave を集計保存 | 条件付き | 遅刻早退、深夜労働は後続                                 |
| 再締め/再出力時の基準スナップショット     | `periodKey + version` で保持                                           | 初期充足 | `reclose=true` で再締め版を追加作成                      |

- `#1442` の baseline として、最新の closed snapshot を canonical 勤怠 CSV として取得・dispatch する route を追加した
...
- 従って、`#1437` と `#1440` では深夜労働、遅刻早退、CSV 列最終確定を後続で詰める必要がある
...
| 仕訳 mapping rule                        | `AccountingMappingRule` 追加                        | 初期充足       | `mappingKey` ごとに debit/credit/tax/department/name を設定し、既存 staging へ再適用できる |
...
- `GET /integrations/reconciliation/summary?periodKey=YYYY-MM` を実装済み
  - 以下の aggregate 差異を返す
  - `AttendanceClosingPeriod` と `AttendanceMonthlySummary` の締め集計
  - latest full `HrEmployeeMasterExportLog` と attendance closing の社員コード差分
  - `AccountingJournalStaging` の `ready / pending_mapping / blocked`
  - latest `AccountingIcsExportLog` の件数と ready 件数の比較
- `GET /integrations/reconciliation/details?periodKey=YYYY-MM` も実装済み
  - 以下の差異詳細を返す
  - payroll の社員コード差分全件
  - accounting の `PJ別` / `部門別` breakdown
  - `pending_mapping` / `blocked` / `invalid ready` の sample 行
- UI は aggregate summary に加えて details drilldown を実装済みで、同一 `periodKey` の社員コード差分、`PJ別 / 部門別` breakdown、`pending_mapping / blocked / invalid ready` sample 行を確認できる
