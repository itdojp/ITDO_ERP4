...
| 分類                         | 現状                                                          | 判定           | 補足                                                         |
| ---------------------------- | ------------------------------------------------------------- | -------------- | ------------------------------------------------------------ |
| 外部連携用社員コード         | `UserAccount.externalId` はあるが、給与連携専用コードではない | 不足           | IdP/SCIM の外部 ID と給与連携キーは分離した方が安全          |
...
| 雇用区分                     | 専用項目なし                                                  | 不足           | 正社員/契約/パート等の給与前提が保持されていない             |
| 支払口座                     | 専用項目なし                                                  | 不足           | `Vendor.bankInfo` はあるが従業員口座は未保持                 |
| 給与体系/締日/基準日数・時間 | 専用項目なし                                                  | 不足           | 給与計算前提を保持するプロファイルが未実装                   |
...
| 月次締め済み勤怠確定値                    | 専用モデルなし          | 不足 | issue #1440 の主対象                                    |
| 締め状態/締め基準日                       | 専用モデルなし          | 不足 | どの時点を payroll export の正とするか保持不可          |
| 出勤日数/残業/遅刻早退/有給消化の確定集計 | 集計 API/保存モデルなし | 不足 | leave export は休暇明細中心で、給与向け月次集計ではない |
| 再締め/再出力時の基準スナップショット     | なし                    | 不足 | 月次再現性の担保ができない                              |

### 2.3 給与向けに現状利用できるもの

- 既存 leave export は「休暇申請の approved 明細を HR 向けに出す」用途には使える
- ただし「給料らくだへ投入する月次勤怠確定 CSV」の代替にはならない
- 従って、`#1437` と `#1440` で月次締めモデル・集計ロジック・CSV 出力前提を新設する必要がある
...
| 仕訳候補/仕訳ヘッダ・明細 staging        | 専用モデルなし                                 | 不足           | issue #1441 の主対象                            |
