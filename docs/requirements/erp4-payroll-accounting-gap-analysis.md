...
- 初期実装として `GET /integrations/reconciliation/summary?periodKey=YYYY-MM` を追加し、以下の aggregate 差異を返す
...
- 次段として `GET /integrations/reconciliation/details?periodKey=YYYY-MM` を追加し、以下の差異詳細を返す
  - payroll の社員コード差分全件
  - accounting の `PJ別` / `部門別` breakdown
  - `pending_mapping` / `blocked` / `invalid ready` の sample 行
- UI は上記 detail API の後続段階に分離する
