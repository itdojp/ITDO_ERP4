...
- `UserIdentity` 未導入の既存データに対しては、移行期間の互換層として `userName` / `externalId` lookup を継続する
  ...
  - DB照会の負荷が問題になる場合はキャッシュを有効化できる: `AUTH_DB_USER_CONTEXT_CACHE_TTL_SECONDS`（秒、0で無効。無効がデフォルト）。identity-backed contextのcache期限は設定TTLと`UserIdentity.effectiveUntil`の早い方とし、identity失効後は再照会して401で拒否する。session認証のcache keyはprovider subjectだけで共有せず、canonical `UserIdentity.id`と`UserAccount.id`で分離し、同じprovider subjectへ紐づく別identity/sessionの正・負contextを再利用しない。SCIMおよび手動group更新・membership変更はDB mutation/transaction commit直後かつ外側監査処理を待つ前にprocess-local cacheを全消去し、identity/credentialの対象更新は同じprovider subjectに属するsession/JWT cacheを一括無効化する。SCIM Group POST/PUT/PATCHのgroup本体と全membership操作は同じPrisma transactionへ束縛し、入力はmutation前に全件検証する。途中失敗時はgroup/membershipをrollbackして失敗操作の外側監査を残さず、成功commit後だけcacheを全消去する。DB context解決中に無効化が完了した場合は、解決開始時のcache write epochと不一致になるため失効前snapshotをcacheへ再挿入しない。複数backend instanceで正のTTLを使うには分散invalidationが必要であり、それを実装するまでは各instanceで0を維持する
  - SCIM user DELETEでは`UserAccount`無効化、個人総務チャットmembership無効化、その副作用監査を同じPrisma transactionへ束縛する。chat副作用またはcommit失敗時はaccount/chat変更とtransaction内監査をrollbackし、成功時はcommit直後かつ外側監査ログより先にprocess-local auth context cacheを全消去する。監査書込み自体の失敗は既存のfail-open契約を維持する。共通side-effect portを使うSCIM PUT/PATCHのchat副作用監査も同じtransaction clientへ束縛する
