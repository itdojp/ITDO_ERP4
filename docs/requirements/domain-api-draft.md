...
- **alert_settings**: `id`, `type`(budget_overrun/overtime/approval_delay/delivery_due), `threshold`, `period`, `recipients`(emails/roles/users), `channels`(email,dashboard,ext_future), `remindAfterHours`。
