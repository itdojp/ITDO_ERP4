...
- 配信ステータス: `pending` / `sending` / `retrying` / `success` / `stub` / `failed` / `failed_permanent` / `skipped`
- 初回 email 配信: `POST /report-subscriptions/:id/run` / `POST /jobs/report-subscriptions/run` でまず `pending` を永続化し、即時処理または retry job が配送する
- 再送ジョブ: `POST /jobs/report-deliveries/retry`（`status=pending` の初回配送、および `status=failed` + `nextRetryAt<=now` の再送を処理）
...
| management-accounting-summary | 管理会計サマリ         | from, to           | -                                   | GET `/reports/management-accounting/summary`      |
