...
| 部署／担当者 | `im_offices`, `users` / `persons` / `parties` | プロジェクトメンバー、承認者を管理                     | 組織更新時の同期が手作業、権限モデルと分離 |
...
3. **外部連携**
   ...
   - Google グループ／組織情報の取り込みは任意とし、初期本番では ERP4 側 DB を `OrgUnit` / `project_members` の一次ソースとする。
   - Google 由来データを使う場合も read-only の補助ソースに限定し、`docs/requirements/auth-architecture.md` の方針を優先する。
...
| `project_members`     | `id`   | `project_id`, `user_id`, `role_code`, `allocation_percentage`, `start_date`, `end_date`, `source`                                                                                                             | プロジェクト配員管理。ERP4 側を一次ソースとし、外部同期は差分照合用途に限定 |
...
- **権限管理**: ユーザーは Google SSO または例外ローカル認証 → ERP4 内ロール付与の流れで権限を得る。Google グループは初期本番では補助情報に留め、マスタ編集権は限定ロール（PMO/経理）が所有。
