...
- app_gdrive_chat / pdf / evidence / report
- backup_local / backup_sakura_primary / backup_gdrive_secondary
- restore_evidence

statusは`pass | warn | fail | unknown | not_configured`、exit codeは`0=pass`、`1=warn`、`2=fail`、`3=unknown/not_configured`、`64=configuration/argument error`とする。優先順位はfail、unknown/not_configured、warn、passであり、primary/secondaryの片方だけの成功を全体passにしない。

monitor既定値はDrive 70% warning / 80% critical、local/Sakura hourly 2時間、Drive daily 30時間、restore evidence 30日、最低世代hourly 48 / daily 30 / weekly 12 / monthly 13とする。envで変更できるが、warningはcritical未満、値はbounded positive decimal integerでなければならない。monitorの最低世代既定値はread-only判定とcandidate計算用であり、retention applyが要求する明示的`RETENTION_MIN_*`と人間承認を省略しない。

Drive quotaのlimit/usageが確定できない場合はunknownとする。Sakuraの定期integrity checkはmanifest、object size、HEAD checksum metadataを検査し、全ciphertextの毎時downloadを行わない。Sakura APIはbounded timeoutとinventory entry上限を持ち、超過やtimeoutを正常扱いしない。full content hashはverified upload/downloadとrestore drillで検証する。

summaryとrecordはcomponent、status、allowlist reason、件数、割合、canonical UTCだけを含める。credential、provider identifier、endpoint、bucket/prefix、object key、backup IDを含めない。timerはread-only oneshotだけを呼び、restore、prune apply、Drive trashを実行しない。
...
- integrated readiness: docs/ops/storage-readiness.md
- readiness template: docs/test-results/storage-readiness-template.md
