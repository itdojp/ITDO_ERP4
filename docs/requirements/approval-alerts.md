...
- 設定項目: `threshold`(金額/率/時間), `period`(day/week/month), `scope`(全体/プロジェクト), `recipients`(emails/roles/users), `channels`(email, dashboard; 将来: slack/webhook), `remindAfterHours`(再送間隔; 未設定なら再送なし)。
...
- アラート: 閾値・期間・通知先・チャネル・再送間隔を設定。無効化/有効化スイッチを持つ。
...
- アラート設定: `予算超過閾値`(110%)、`残業閾値`(1h/日 または 5h/週)、`承認遅延閾値`(24h)、`承認期限エスカレーション閾値`(例 48h)、`納期超過未請求`(1件以上)、`通知先`(メール/ダッシュボード/ロール指定)、`リマインド間隔`(例 24h; 未設定なら再送なし)。
