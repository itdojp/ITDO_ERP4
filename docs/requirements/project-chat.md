...
- reply POSTは非冪等であるため、transport failureまたは不整合2xxで結果を確認できない場合は同一panelでの再送を禁止する。POST response本文をthreadへ直接合成せず、fresh thread GETで同一reply IDを確認できた場合だけ表示する。先頭50件外、logical delete、ACL失効、refresh failureで確認できない場合は固定文言で再読み込みを案内する。
...
- `chat_message` deep linkはadditiveな`parentMessageId` / `threadRootId`をallowlist normalizeする。reply topologyではcanonical threadを直接開き、root timelineへreplyを重複表示しない。不完全・cross-root topologyはfail closedとする。
