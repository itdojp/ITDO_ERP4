...
- reply POSTは非冪等であるため、transport failure、5xx、または不整合2xxで結果を確認できない場合は同一panelでの再送を禁止する。serverが明示的な4xxでmutation前の拒否を返した場合だけ、draftを保持して修正・再送を許可する。POST response本文をthreadへ直接合成せず、fresh thread GETで同一reply IDを確認できた場合だけ表示する。先頭50件外、logical delete、ACL失効、refresh failureで確認できない場合は固定文言で再読み込みを案内する。
