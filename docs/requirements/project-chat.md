...
- room selector、room event、root deep linkのいずれで別roomへ移動する場合も、表示済みの旧threadを先に閉じ、対象roomのroot timeline取得が成功するまで旧thread本文を残さない。thread取得・thread mutation・reply/ACK reply POST・root POST・attachment upload/download・root timeline上のreaction/ACK/revoke/cancel・表示済み範囲の既読更新が403/404になった場合は、mutation開始時のroom identityが現在roomと一致することを確認してから表示済みthreadとglobal searchを破棄し、current room timelineを無filter・read-state副作用なしで再取得してread ACLを再検証する。mutation lockはroom ACL再検証と必要なthread再取得が終わるまで解除しない。room readが成功した場合だけ最新timelineを保持し、失敗した場合はtimeline、thread、unread、filter、global searchを安全な固定文言とともに破棄し、先行したthread再取得responseを再適用しない。遅延した旧roomのaccess callbackは現在room stateを変更しない。対象replyだけが同時削除され、thread再取得とroom readが成功した場合は、最新stateを表示してroom全体の失効と混同しない。
