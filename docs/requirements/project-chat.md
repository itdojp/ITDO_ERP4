...
- 別roomのroot deep linkを開く場合も、表示済みの旧threadを先に閉じ、対象roomのroot timeline取得が成功するまで旧thread本文を残さない。thread表示中のreaction、ACK、取消、削除が403/404になった場合はcurrent threadを再取得し、root/room accessも失効していることを確認した時点で表示済みthread本文を破棄する。対象replyだけが同時削除された場合は、再取得した最新threadを表示してroom全体の失効と混同しない。
