...
- frontendはreply/ACK POST responseを要求roomとcanonical rootへbindし、ACK mutation responseはrequest ID、message ID、room ID、子ackのrequest IDを全て検査する。候補responseはallowlist fieldと有限件数・文字長で正規化し、unknown fieldをstateへ保持しない。
- root message / root確認依頼 / reply POSTは非冪等であるため、transport failure、5xx、または不整合2xxで結果を確認できない場合は同じcomposerからの再送を禁止する。root POSTのlifecycleは`RoomChat` sectionではなくApp sessionが所有し、section unmount/remountや単純なroom state変更でlockを解除しない。root POST中と結果不明後はroom selector、room deep link、room作成、root timeline操作と新規thread openを無効化する。POST開始時のroomへrequestを固定し、異なるroomで完了した結果を現在のcomposerへ反映しない。POST待機中にsectionがunmountされた場合は、結果だけをApp sessionへ確定し、添付upload、timeline refresh、read mutation等の新しい副作用を開始しない。結果不明時はbrowser page reloadだけを再開境界とする。serverが明示的な4xxでmutation前の拒否を返した場合だけ、draftを保持して修正・再送を許可する。POSTが成功した後の添付uploadまたはtimeline refresh失敗はmessage作成失敗と混同せず、draftを消去して「messageを再送しない」固定案内を表示する。replyのPOST response本文はthreadへ直接合成せず、fresh thread GETで同一reply IDを確認できた場合だけ表示する。先頭50件外、logical delete、ACL失効、refresh failureで確認できない場合は固定文言で再読み込みを案内する。
...
- room selector、room event、root deep linkのいずれで別roomへ移動する場合も、表示済みの旧threadを先に閉じ、対象roomのroot timeline取得が成功するまで旧thread本文を残さない。thread取得・thread mutation・reply/ACK reply POST・root POST・root timeline上のreaction/ACK/revoke/cancelが403/404になった場合は、mutation開始時のroom identityが現在roomと一致することを確認してから表示済みthreadとglobal searchを破棄し、current room timelineを無filterで再取得してread ACLを再検証する。room readが成功した場合だけ最新timelineを保持し、失敗した場合はtimeline、thread、unread、filter、global searchを安全な固定文言とともに破棄する。遅延した旧roomのaccess callbackは現在room stateを変更しない。対象replyだけが同時削除され、thread再取得とroom readが成功した場合は、最新stateを表示してroom全体の失効と混同しない。
