...
- room selector、room event、root deep linkのいずれで別roomへ移動する場合も、表示済みの旧threadを先に閉じ、対象roomのroot timeline取得が成功するまで旧thread本文を残さない。thread取得・thread mutation・root POSTが403/404になった場合は表示済みthreadを直ちに破棄し、current room timelineを無filterで再取得してread ACLを再検証する。room readが成功した場合だけ最新timelineを保持し、失敗した場合はtimeline、unread、filter、global searchを安全な固定文言とともに破棄する。対象replyだけが同時削除され、thread再取得とroom readが成功した場合は、最新stateを表示してroom全体の失効と混同しない。
