...
5. `provider=google_oidc`, `subject=sub` の `UserIdentity` を検索し、対応する `UserAccount` に解決する。
...
- `UserIdentity`
  ...
  - `providerSubject` (`sub`、またはローカル loginId)
  - `issuer`
  - `emailSnapshot`
  - `status`
  - `lastAuthenticatedAt`
...
- ローカルユーザの識別は `UserIdentity(providerType=local_password, providerSubject=loginId)` で行う。
...
- 現行 `docs/requirements/id-management.md` には `externalId=issuer + sub` の記述があるが、実装は `sub` ベースで照合している箇所がある。
...
- `UserIdentity` / `LocalCredential` を導入する。
- 例外ユーザ向けローカル認証を実装する。
