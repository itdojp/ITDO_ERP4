...
- Google グループは初期状態では権限制御の一次ソースにしない。利用する場合も read-only・粗い権限マッピングに限定する。
  - 詳細条件は `docs/requirements/google-workspace-group-usage-policy.md` を参照する。
...
- backend route 構成は、HTTP登録の変更単位を小さくするため以下に分割する。
  ...
  - `packages/backend/src/routes/auth/googleSessionRoutes.ts`
    ...
    - session list / revoke
    - logout
  - `packages/backend/src/routes/auth/http.ts`
    - `jwt_bff` 有効判定
...
- Phase 1 backend では次を提供する。
  ...
  - local credential 発行時の既定値は `mfaRequired=true` とする
    - MFA setup / challenge 経路が未実装のため、既定発行 credential は初回再設定後も session を発行しない
    - system_admin は例外的に `mfaRequired=false` を指定できるが、password-only 例外は `reasonText` を必須とし監査ログへ `mfaDefaultOverridden=true` を記録する
...
- Google グループだけで `system_admin` / `admin` / `mgmt` / `exec` を即時付与しない。
...
- `LocalCredential`
  ...
  - `passwordChangedAt`
  - `recoveryCodesHash`
- `loginId` は運用上変更可能な表示識別子であり、認証主体の不変識別子は `providerSubject` とする。
...
- セッションの初期値
  ...
  - idle timeout: 2 時間
- OIDC 認可フロー state/nonce/code_verifier の保持期間は 10 分を初期値とする。
- 実装上の環境変数は以下を用いる。
  - `AUTH_SESSION_COOKIE_NAME`
  - `AUTH_SESSION_FLOW_COOKIE_NAME`
  - `AUTH_SESSION_COOKIE_SECURE`
  - `AUTH_SESSION_ABSOLUTE_TTL_HOURS`
  - `AUTH_SESSION_IDLE_TTL_MINUTES`
  - `AUTH_SESSION_FLOW_TTL_MINUTES`
