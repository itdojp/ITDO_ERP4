...
- 選択可能sourceはexactなsnapshot、annotation revision、conversation turn、synthesis version、thread-promotion messageとする。各sourceのcurrent ACL、scope、logical deletion、version/hashをpreview、commit、detail、reconcileで再検査する。personal sourceはowner本人だけ、organization sourceは同一organizationの有効group grantを含む通常read ACLを満たすactorだけが選択できる。conversation turnは`user|assistant`だけをMVPで外部送信可能とし、`system|tool`は明示的な後続契約なしに選択できない。synthesis provenanceはdepth 1、全source 32、関連item 10、一source 64 KiB、合計256 KiB、利用者prompt 16 KiB、output 4,096 token、永続化するresult 256 KiBを上限とする。
