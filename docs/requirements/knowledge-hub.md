...
- browser captureは独立したManifest V3 packageとしてbuildする。生成manifestのpermissionは`activeTab|scripting|storage`だけとし、`incognito: not_allowed`でChromeのシークレットモード／Edge InPrivateを明示的に対象外とする。`<all_urls>`、wildcard host permission、`cookies`、`tabs`、`history`、`webRequest`、clipboard、native messagingを要求しない。remote code、inline executable script、`eval`、`innerHTML`、browser cookie／ERP4 tokenの取得、ERP4 APIへの直接requestを禁止する。
...
- extension-local storage lifecycleは`staged|pending|cleanup_pending`に限定し、commit I/O前にbackend正規化済みdraftとintentをsession storageへ固定する。同じoperationだけがidempotentに再開でき、別payload／actor／nonceを拒否する。pending書込み後のbridge responseが不明な場合はoperation IDをcomponent memoryだけに保持し、自動送信せず、利用者の明示retryだけが同じoperation／intent／draftを再提示してcommitへ進める。別operationはCAS loserとしてread-only確認へ移し、同じoperationのpayload不一致はfail closedとする。page bridgeは新規pending成功時にoperation ID、intent、draftの完全一致を検証する。draft TTL内に受理したnonceは最大32件の履歴からevictせず、上限到達後のcontent-bearing commandはfail closedとする。commit、確定failed、discard後は、request key、draft、pending intentを含まないallowlist済み`cleanup_pending` tombstoneへ一回のsession-storage writeで先に置換し、その後に物理削除する。extension `get`の前に、ERP4 same-origin storageへschema、opaque draft ID、最大10分の固定expiry、1文字stateだけを持つcontent-free terminal fenceを独立2 slotで同期予約する。初期化が一部だけ成功したactive slotはterminalと解釈せずfail closedとする。terminal result時は既存slotと同じ固定長valueを`active`から`terminal`へ上書きし、どちらか一方のverified terminal slotだけで単調なterminal状態を確立するため、新規容量確保や単一write成功へ依存しない。current addressからopaque handoff parameterを外して、storage消去済みを意味しないlocal `terminal`通知を他tabへ送る。app local storageへcapture本文、request key、actor、intent、preview tokenは保存しない。受信中のtabは通知で、reload中／後発tabはextension `get`前のdual-slot state読取でDOMとaddressをpurgeする。`get`がterminal遷移と並行した場合も、response受領後かつdraft event発行前にfenceを再検査して本文を破棄する。terminal fenceは物理削除成功後もbounded TTLまで保持し、terminal slotをreloadでactiveへ戻さない。expiry後はextensionの同じ10分read TTLがstale contentを拒否し、app起動時の全key scan／最大100件削除がexpiredなcontent-free slotだけをbest-effort削除する。tombstone書込み失敗と物理削除失敗を別のUI phaseとして表示し、前者ではbrowser session内本文の消去済みを主張しない。tombstone書込みが失敗してもidempotent deleteを一度試し、両方が失敗した場合だけ明示cleanup stateを保持する。tombstone遷移またはdeleteのresponse lossは同じactorのcontent-free commandへidempotentに収束し、物理削除失敗後のreloadでも本文、request key、preview／commit操作を復元しない。認証／network喪失時もcleanup状態を上書きせず、既claim draftのtombstone化／物理削除に限り最後に検証済みのactor keyをcomponent memoryへ保持する。このkeyをAPI、content read、staged/pending復元には使用しない。結果不明では自動再送せずread-only reconcileを表示し、terminal cleanup失敗時は保存mutationではなくcontent-free tombstone化／削除だけを明示再試行する。page bridgeはresponse envelopeだけでなくnon-null inner recordのdraft IDもrequested IDへ束縛し、command別に`transitioned`、record lifecycle、operation／intent／draftを検証して、意味的に不整合なsuccess responseをfail closedで拒否する。
