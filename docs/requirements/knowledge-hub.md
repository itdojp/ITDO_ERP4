...
- audit metadataはscope、status、version、変更field名とboundedなprincipal/actor/scope provenanceだけを許可し、本文、canonical URL、query、reason text、token ID、secretを含めない。request IDはserverと同じ安全文字・128文字上限を再検証し、不一致時は保存しない。利用者が指定できる自由文であるUser-AgentはKnowledge auditへ保存しない。
