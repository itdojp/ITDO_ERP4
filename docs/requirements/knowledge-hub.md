...
- hard判定は`settled actual + active reservation + held maximum + new maximum`で行う。active policyの上限を適用しつつ、同じsubjectの当月消費とrolling 60-minute request数はinactiveな旧policy versionのreservationも含める。policy/periodを決定順にlockし、Serializable transactionを最大3 attemptだけ再評価する。同じopaque request hashは一つのrun/reservationへ収束し、hard/rate blockではprovider requestを作らない。
- conservative input estimateは各sourceについて`UTF-8 bytes * 2 + 16 framing tokens`でありprovider tokenizer実測ではない。dispatch時にDBがsourceごとのbyte数とestimate、source種別別件数、合計256 KiB、関連item 10件、provenance depth 1を再検証し、runのinput estimateを超える集合を拒否する。runはcatalog versionに加えてinput/output単価をimmutable snapshotとして保持し、実usageはprovider outcomeとassistant/AI turnへ束縛してsettlement時にstrictに再検証する。costはcaller入力を信用せず、snapshot単価から各項を`ceil(tokens * price / 1,000,000)`で再計算する。
