...
- extension-local lifecycleは`staged|pending`に限定し、commit I/O前にbackend正規化済みdraftとintentをsession storageへ固定する。同じoperationだけがidempotentに再開でき、別payload／actor／nonceを拒否する。結果不明では自動再送せずread-only reconcileを表示する。commit、確定failed、discard後はsession draftを削除し、削除失敗時は再送ではなく削除だけを明示再試行する。
