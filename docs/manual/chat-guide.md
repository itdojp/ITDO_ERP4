...
- 投稿結果を確認できない場合は、同じpanelからの再送が無効になります。二重投稿を避けるため再送せず、panelを閉じて再読み込みしてください。入力不備等がserverの4xxで明示的に拒否された場合はdraftを保持したまま修正・再送できます。投稿後の再取得で同じ返信を確認できない場合（返信が51件目以降にある場合を含む）も、POST応答の本文を暫定表示せず案内を表示します。
