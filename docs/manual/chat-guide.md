...
- 親message、親の確認依頼、返信の投稿結果を確認できない場合は、同じcomposerからの再送が無効になります。二重投稿を避けるため再送せず、ページまたはpanelを再読み込みしてください。入力不備等がserverの4xxで明示的に拒否された場合はdraftを保持したまま修正・再送できます。投稿自体の成功後に添付または一覧更新だけが失敗した場合はdraftを消去し、messageを再送しない案内を表示します。投稿後の再取得で同じ返信を確認できない場合（返信が51件目以降にある場合を含む）も、POST応答の本文を暫定表示せず案内を表示します。
...
- 別ルームの親messageをdeep linkで開く場合は、現在のthreadを閉じてから対象ルームへ移動します。thread操作中に権限が失効した場合は再取得で確認後に表示済みthread本文を消去し、権限外本文を画面へ残しません。
