...
- 親message、親の確認依頼、返信の投稿結果を確認できない場合は、同じcomposerからの再送が無効になります。二重投稿を避けるため再送せず、ページまたはpanelを再読み込みしてください。親message／確認依頼の投稿中または結果不明後はルーム切替・deep link・ルーム作成も無効になり、別ルームへ同じdraftを誤送信できません。入力不備等がserverの4xxで明示的に拒否された場合はdraftを保持したまま修正・再送できます。投稿自体の成功後に添付または一覧更新だけが失敗した場合はdraftを消去し、messageを再送しない案内を表示します。投稿後の再取得で同じ返信を確認できない場合（返信が51件目以降にある場合を含む）も、POST応答の本文を暫定表示せず案内を表示します。
...
- ルーム一覧、ルーム通知、別ルームの親message deep linkのいずれから移動する場合も、現在のthreadを閉じてから対象ルームへ移動します。thread操作中に権限が失効した場合はthread本文を消去し、ルーム一覧を再取得して閲覧可能なら最新timelineへ置換します。ルーム自体を閲覧できない場合はtimelineと横断検索結果も消去し、権限外本文を画面へ残しません。
