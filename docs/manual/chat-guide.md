...
- 親message、親の確認依頼、返信の投稿結果を確認できない場合は、同じcomposerからの再送が無効になります。二重投稿を避けるため再送せず、browser pageを再読み込みしてください。panelを閉じて開き直した場合やチャット画面を離れて戻った場合もlockは解除されません。親message／確認依頼／返信の投稿中または結果不明後はルーム切替・deep link・ルーム作成も無効になり、別ルームへ同じdraftを誤送信できません。入力不備等がserverの4xxで明示的に拒否された場合はdraftを保持したまま修正・再送できます。投稿自体の成功後に添付または一覧更新だけが失敗した場合はdraftを消去し、messageを再送しない案内を表示します。投稿後の再取得で同じ返信を確認できない場合（返信が51件目以降にある場合を含む）も、POST応答の本文を暫定表示せず案内を表示します。
