...
- スレッド内では通常返信と確認依頼付き返信を投稿でき、既存のメンション、リアクション、確認（OK）契約をそのまま利用します。
- 横断検索で返信がヒットした場合は `返信` ラベルを表示し、`スレッドを開く` で対応する親と返信履歴を表示します。
- 親または返信を論理削除した場合、本文・タグ・添付・メンション等は表示せず、削除済みのプレースホルダーを表示します。親を削除しても既存返信の監査履歴は保持され、返信件数には削除済みプレースホルダーも含まれます。
- スレッドを開いて表示した範囲だけを既読境界として送信します。未取得の新しい返信を、画面表示だけで既読にしません。
- 投稿や操作が完了した後の表示更新だけに失敗した場合は、二重投稿を避けるため再送せず、案内に従って再読み込みしてください。読み込み済みの返信は表示上保持されます。
