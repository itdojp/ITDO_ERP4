...
- 目的: 休暇の申請と一覧確認
- 主な操作: 休暇種別/期間/時間/備考の入力 → 作成 → （必要に応じて相談証跡を添付）→ 申請
...
4. （任意）`詳細` を開き `相談証跡/メモ` にチャット参照（`chat_message`）を添付する
5. 証跡を添付しない場合は `相談無し` をチェックし、理由を入力する
6. 一覧の `申請` を押して申請状態に更新する
7. 工数重複がある場合は「工数の重複」に該当工数が表示される
8. 有給不足の場合は、申請完了後に不足分の警告メッセージが表示される
