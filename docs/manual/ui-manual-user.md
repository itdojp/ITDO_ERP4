# ERP4 PoC UI 詳細操作マニュアル（利用者）

## 前提

- 本書は PoC 環境の UI 操作に関する詳細ガイドです。
- 短時間で全体導線を確認する場合は `docs/manual/user-tutorial.md` を先に参照してください。
- 画面キャプチャは 2026-03-09 実行の E2E（r1）で取得しています。
  - 証跡: `docs/test-results/2026-03-09-frontend-e2e-r1/`
- UI証跡の再取得手順: [ui-evidence-quickstart](ui-evidence-quickstart.md)（詳細: [e2e-evidence-howto](e2e-evidence-howto.md)）
- Push通知の証跡のみ `docs/test-results/2026-01-19-frontend-e2e-pwa-push/` で取得しています。
- 画面の表示内容は demo seed に基づきます（データ差分あり）。
- 対象ロールの目安: user
- 管理者向けの操作は `docs/manual/ui-manual-admin.md` を参照してください。

## 目次

- 共通: 現在のユーザー / ダッシュボード / ERP横断検索 / コマンドパレット / Knowledge Hub
- 利用者向け: 日報 / 工数 / タスク / 経費 / 休暇 / 見積 / 請求 / チャット / オフライン
- 補足: PWA キャッシュ更新

## 共通: 現在のユーザー

- 目的: 現在のログイン情報、認証セッション、通知/オフラインキューの状態確認
- 主な操作: 簡易ログイン/ログアウト、認証セッション確認/失効、Push同意、オフライン再送
- 補足: `VITE_AUTH_MODE=jwt_bff` の場合は backend の Auth Gateway へ遷移する `Googleでログイン` ボタンと、例外ユーザ向けのローカル認証フォームを表示します
- 補足: `VITE_AUTH_MODE` 未設定時は、`VITE_GOOGLE_CLIENT_ID` 設定時のみ Google Identity Services のボタンを表示します
- 証跡: `docs/test-results/2026-03-23-frontend-e2e-r2/00-current-user-auth-sessions.png`

### 詳細操作

1. 未ログイン時に `userId` / `roles`（例: `admin,mgmt`）を入力し「簡易ログイン」を押す
2. 任意で `projectIds` / `groupIds` を入力し、認可対象を調整する
3. ログイン済みであれば `ID / Roles / OwnerProjects / Groups` を確認する
4. 「通知設定」でメール通知（`realtime` / `digest`）と配信間隔（分）を設定する
5. 「通知設定」で全体ミュートの期限を入力し `保存` を押す（任意）
6. 「オフライン送信キュー」で `再送` もしくは `再読込` を実行する
7. キュー明細で `再送` / `破棄` を選択する
8. 「Push通知」で配信条件を選択し、同意チェック後に `購読登録` / `購読解除` / `テスト通知` を実行する
9. `VITE_AUTH_MODE=jwt_bff` では、必要に応じて `ローカル認証 loginId` / `password` を入力し `ローカルログイン` を実行する
10. 初期パスワード更新が要求された場合は `ローカル認証 new password` を入力し `初期パスワードを更新` を実行する
11. `VITE_AUTH_MODE=jwt_bff` では、`認証セッション` 一覧で current / other session を確認し、必要に応じて `このセッションを終了` / `このセッションを失効` を実行する
12. `ログアウト` を押してローカル認証情報を破棄する

### 入力項目/制約

- `VITE_AUTH_MODE=jwt_bff` の場合、未ログイン時は簡易ログインを表示せず、backend の Auth Gateway 経由の Google 認証と、例外ユーザ向けローカル認証フォームを表示します
- ローカル認証フォームは例外ユーザ専用です。Google を利用できるユーザは Google 認証を優先します
- 初期パスワードの更新が必要な credential では、`初期パスワードを更新` 完了後に新しいパスワードで再度ログインします
- `mfaRequired=true` の credential は、MFA 実行経路が未実装のため session を発行しません。管理者発行のローカル credential は既定でこの状態になります
- `認証セッション` 一覧は `VITE_AUTH_MODE=jwt_bff` かつログイン済みの場合のみ表示されます
- `このセッションを終了` は現在の browser session を失効し、即時ログアウトします
- `VITE_AUTH_MODE` 未設定時の Googleログインは `VITE_GOOGLE_CLIENT_ID` が未設定の場合は表示されません
- メール通知は `digest` の場合、配信間隔（分）が必要です
- メール通知の既定は `digest` / 10分です
- Push 通知は `VITE_PUSH_PUBLIC_KEY` 未設定だと購読登録に失敗します
- Push自動配信の通知種別は backend の `NOTIFICATION_PUSH_KINDS` で制御されます
- `VAPID_*`（backend）が未設定の場合、`テスト通知` は実配信ではなくローカル表示になります
- 送信待ちキューはログイン済みの場合のみ処理できます

![現在のユーザー](../test-results/2026-03-09-frontend-e2e-r1/00-current-user.png)

### Push通知（証跡）

`VITE_PUSH_PUBLIC_KEY` が設定されている環境でのみ取得しています。

![Push購読（登録）](../test-results/2026-01-19-frontend-e2e-pwa-push/17-push-registered.png)

![Push購読（解除）](../test-results/2026-01-19-frontend-e2e-pwa-push/18-push-unsubscribed.png)

![Push購読（再登録）](../test-results/2026-01-19-frontend-e2e-pwa-push/19-push-resubscribed.png)

---

## 共通: ダッシュボード

- 目的: アラート・承認・通知・インサイトの概要確認
- 主な操作: 件数確認、通知の既読化、アラート/インサイトの一覧確認
- 補足: 通知には案件作成/ステータス変更などの運用イベントが含まれます
- 補足: インサイトは権限により表示されない場合があります

### 詳細操作

1. 「承認状況」で承認待ち件数と自分の承認待ち件数を確認する
2. 「通知」で対象通知の `既読` を押して未読を消し込む
3. 「Alerts」で `すべて表示` / `最新のみ` を切り替える
4. 「Insights」で件数・期間・根拠・ヒントを確認する（admin/mgmt/execのみ）

![ダッシュボード](../test-results/2026-03-09-frontend-e2e-r1/01-core-dashboard.png)

---

## 共通: ERP横断検索

- 目的: 案件/見積/請求/工数/経費/仕入/チャットの横断検索
- 主な操作: 検索語入力 → 検索
- 補足: 2文字以上で検索を実行します

### 詳細操作

1. 「検索語」に 2 文字以上のキーワードを入力する
2. 「取得件数/種別」で取得件数を指定する（1〜50）
3. `検索` を押して結果を取得する
4. チャット結果は `開く` で該当チャットを開く
5. `クリア` で入力と結果を初期化する

![ERP横断検索](../test-results/2026-03-09-frontend-e2e-r1/06-core-global-search.png)

---

## 共通: コマンドパレット

- 目的: 主要画面への移動と主要操作（再取得/新規作成/検索）をキーボード中心で実行する
- 主な操作: `Ctrl+K`（Windows/Linux）または `Cmd+K`（macOS）で起動し、検索して実行

### 詳細操作

1. `Ctrl+K` / `Cmd+K` でコマンドパレットを開く
2. 画面名（例: `ルームチャット`）または操作名（例: `再取得`）を入力する
3. 候補を選択して `Enter` で実行する
4. `Esc` で閉じる

---

## 共通: Knowledge Hub

- 目的: 外部情報や手動メモを個人または明示した組織scopeのInboxへ保存し、immutableなversion・SHA-256・出所を確認する
- 主な操作: text / URL / PDF / imageの保存、version追加、保存結果の再照合、認可済みdownload
- 詳細: [Knowledge Hub 保存・annotation・会話・Synthesisガイド](knowledge-hub.md)
- snapshot証跡: `docs/test-results/2026-08-06-issue2012-knowledge-snapshot-ui/01-knowledge-hub-manual-capture.png`
- provenance証跡:
  - `docs/test-results/2026-08-08-issue2013-knowledge-provenance-ui/01-annotation-revision-history.png`
  - `docs/test-results/2026-08-08-issue2013-knowledge-provenance-ui/02-conversation-role-timeline.png`
  - `docs/test-results/2026-08-08-issue2013-knowledge-provenance-ui/03-synthesis-version-provenance.png`

![Knowledge Hub](../test-results/2026-08-06-issue2012-knowledge-snapshot-ui/01-knowledge-hub-manual-capture.png)

![本人annotationの改訂履歴](../test-results/2026-08-08-issue2013-knowledge-provenance-ui/01-annotation-revision-history.png)

![会話のroleとorigin timeline](../test-results/2026-08-08-issue2013-knowledge-provenance-ui/02-conversation-role-timeline.png)

![Synthesisのversionとprovenance](../test-results/2026-08-08-issue2013-knowledge-provenance-ui/03-synthesis-version-provenance.png)

---

## 利用者向け（一般ユーザ）

### 日報 + ウェルビーイング

- 目的: 日報の記録とコンディション申告
- 主な操作: Good/Not Good 選択、メモ入力、送信
- 補足: Not Good 選択時のみタグと相談フラグが表示されます

### 詳細操作

1. 「日報本文（任意）」に日報を入力する
2. 「関連案件（任意・複数選択可）」で該当案件を選択する
3. `Good` / `Not Good` を選択する
4. `Not Good` の場合はタグ選択、メモ入力、相談チェックを任意で設定する
5. 「ヘルプ / 相談したい」からヘルプを開く
6. `送信` を押して日報とウェルビーイングを登録する
7. 「日報履歴」で `履歴を読み込み` を実行する

### 入力項目/制約

- `Not Good` 時のタグ/メモは任意です
- 管理者（admin/mgmt）は userId で履歴を絞り込み可能です
- 日報/ウェルビーイングの入力は評価目的で利用しません
- オフライン時は送信待ちキューに保存されます

![日報](../test-results/2026-03-09-frontend-e2e-r1/02-core-daily-report.png)

### 工数入力

- 目的: 案件・タスク単位で工数を記録
- 主な操作: 案件/タスク/日付/工数/作業種別/場所の入力 → 追加
- 補足: 15分単位・最大 1440 分の制限があります

### 詳細操作

1. 「案件」で対象案件を選択する
2. 「タスク」は任意で選択する（案件選択後に候補が表示）
3. 「日付」「工数 (分)」を入力する
4. 「作業種別」「場所」を任意で入力する
5. `追加` を押して工数を保存する

### 入力項目/制約

- 工数は 1〜1440 分、15 分単位
- 案件・日付は必須
- オフライン時は送信待ちキューに保存されます

![工数入力](../test-results/2026-03-09-frontend-e2e-r1/03-core-time-entries.png)

### タスク

- 目的: 案件タスクの登録・進捗・ベースライン確認
- 主な操作: 案件選択、タスクの追加/更新、ベースラインの閲覧
- 補足: 依存関係や進捗率はタスク編集で更新します

### 詳細操作

1. 「案件選択」で対象案件を選択し `読み込み` を押す
2. 「ベースライン」で `ベースライン作成` / `一覧更新` を実行する
3. 「新規作成」でタスク名・ステータス・進捗率・日付を入力し `作成`
4. 既存タスクは `編集` から更新し `更新` / `キャンセル` を選択
5. 編集時に「先行タスク選択」で依存関係を設定する
6. 親タスクを変更した場合は「親タスクの変更理由」を入力する
7. 管理者（admin/mgmt）は `削除` を実行できる（理由入力必須）

### 入力項目/制約

- 親タスク変更時は理由入力が必須
- 進捗率は 0〜100
- `削除` は admin/mgmt のみ

![タスク](../test-results/2026-03-09-frontend-e2e-r1/21-project-tasks.png)

### 経費入力

- 目的: 経費の登録と領収書リンクの管理
- 主な操作: 案件/区分/金額/通貨/日付の入力 → 追加、領収書 URL の登録
- 補足: 共通経費は「共通経費」チェックで登録できます

### 詳細操作

1. 「案件選択」で対象案件を選択する
2. 「区分」「金額」「通貨」「日付」を入力する
3. 「共通経費」に該当する場合はチェックする
4. 「領収書URL」を任意で入力し `追加` を押す
5. `状態` / `精算` / `領収書` / `支払日(開始・終了)` で一覧を絞り込む（必要時）
6. `未登録のみ表示` / `全件表示` で領収書未登録の絞り込みを切り替える
7. （admin/mgmt）一覧の `支払済みにする` / `支払取消` で精算状態を更新する
8. 一覧の `注釈` を開き、メモ・外部URL・内部参照を編集する（必要時）
9. `保存` 後に `閉じる` で経費一覧へ戻る

### 入力項目/制約

- 金額は 1〜10,000,000
- 通貨は 3 文字の英大文字（例: JPY）
- 承認依頼時は、`領収書URL` または `添付証憑` のどちらかが必須
- 予算超過時は、申請時にエスカレーション項目（理由/影響/代替案）の入力が必須
- `支払済みにする` は `approved` 状態の経費のみ実行可能（admin/mgmt）
- `支払取消` は理由入力が必須（admin/mgmt）
- オフライン時は送信待ちキューに保存されます

### 補足（注釈）

- 注釈画面では `メモ（Markdown）`、`外部URL`、`内部参照` を一括で管理できます
- `内部参照` の `エビデンス追加` から、チャット発言を画面遷移なしで検索・追加できます
- `エビデンス追加` では `案件/ルーム/投稿日時/投稿者/抜粋` を確認しながら選択できます
- `内部参照` は `候補検索` で候補を追加できます（案件スコープ: 同一案件・親子案件）
- `手動追加（deep link / kind:id）` による追加にも対応しています
- 旧 `project_chat` の deep link / `kind:id` を貼り付けた場合も、保存時は `room_chat` として正規化されます
- `参照状態を確認` で `chat_message` の参照可否（参照可能/権限不足/発言なし）を確認できます
- 推奨手順:
  1. `エビデンス追加` を開き、キーワード検索で候補を表示
  2. `追加` で内部参照に登録（必要なら `メモへ挿入` でMarkdownリンクも追加）
  3. `参照状態を確認` を実行し、`参照可能` を確認してから `保存`
  4. 候補が見つからない場合は `手動追加（deep link / kind:id）` を使用
- 承認者は承認画面の `エビデンス（注釈）` から、同じ参照をコンテキストを維持したまま確認できます
- 申請〜承認〜精算の詳細運用は [expense-workflow-guide](expense-workflow-guide.md) を参照してください

![経費入力](../test-results/2026-03-09-frontend-e2e-r1/04-core-expenses.png)

### 休暇

- 目的: 休暇の申請と一覧確認
- 主な操作: 休暇種別/期間/時間/備考の入力 → 作成 → （必要に応じて相談証跡を添付。非公開相談は「総務へ相談チャットを開く」導線を利用）→ 申請
- 補足: 工数重複や既存休暇との期間重複がある場合は申請エラーとなります。相談証跡が未添付の場合、申請時に「相談無し」の確認と理由入力が必須です。
- 有給の場合、`有給残高（概算）` カードに付与/消化/引当/残高が表示されます。申請時に不足する場合は申請自体は継続しつつ警告が表示されます（運用確認用）。

### 詳細操作

1. `読み込み` で既存の休暇申請を取得する
2. 「申請単位」（終日/時間休）と「休暇種別」「開始日」「終了日」を入力する
3. 終日の場合は任意で「休暇時間（整数）」を、時間休の場合は「開始時刻」「終了時刻」を入力して `作成`
4. （任意）`詳細` を開き `総務へ相談チャットを開く` で個人総務チャットへ遷移する（公開チャットに書けない内容の相談向け）
5. （任意）`詳細` を開き `相談証跡/メモ` にチャット参照（`chat_message`）を添付する
6. 証跡を添付しない場合は `相談無し` をチェックし、理由を入力する
7. 一覧の `申請` を押して申請状態に更新する
8. 工数重複がある場合は「工数の重複」に該当工数が表示される
9. 既存の申請（`pending_manager` / `approved`）と期間重複する場合は `LEAVE_REQUEST_CONFLICT` で失敗する
10. 休暇種別で `承認不要` が設定されている場合、申請後の状態が `approved` へ自動更新される
11. 有給不足の場合は、申請完了後に不足分の警告メッセージが表示される

### 入力項目/制約

- 終日の休暇時間は整数のみ
- 時間休は開始時刻/終了時刻が必須で、同一日かつ設定された最小単位（例: 10分）に合わせて入力
- 時間休の工数超過判定は、その日の所定労働時間（個人上書き > 会社休日=0分 > 既定値）の順で計算されます
- 申請は `draft` 状態のみ
- 休暇種別で `承認不要` が設定されている場合、`申請` 実行後に状態は `approved` へ自動遷移します
- 休暇種別ごとに「申請リード日数（開始日までに必要な最低日数）」と「事後申請可否/上限日数」が設定される場合があります
- 期限外の申請では `LEAVE_SUBMIT_LEAD_DAYS_REQUIRED` / `LEAVE_RETROACTIVE_SUBMIT_FORBIDDEN` / `LEAVE_RETROACTIVE_LIMIT_EXCEEDED` エラーが返ります
- 既存休暇との重複は `LEAVE_REQUEST_CONFLICT`、終日申請中の工数重複は `TIME_ENTRY_CONFLICT`、時間休+工数の超過は `TIME_ENTRY_OVERBOOKED` で失敗します
- `承認不要` 種別でも、重複チェック/期限チェック/証跡要件（`ATTACHMENT_EVIDENCE_REQUIRED`）/相談無し理由チェックは省略されません
- 代休・振替休の残高不足時は `LEAVE_COMP_BALANCE_SHORTAGE` で申請できません
- 上長（プロジェクト管理者）は `上長向け一覧` から `status != draft` の申請を参照できます（理由は表示されません）
- 有給不足は `PAID_LEAVE_*` 警告として表示されます（申請はブロックされません）

### 休暇（総務向け: 有給付与管理）

- 対象: `general_affairs` グループ所属ユーザ
- 目的: 有給付与基準日の管理と付与台帳登録

#### 詳細操作

1. `総務向け: 有給付与管理` で対象ユーザIDを入力する
2. `基準日` と必要に応じて `次回付与予定日` を入力し `プロファイル更新`
3. `付与分数` `付与日`（任意）`失効日`（任意）`理由（必須）` を入力し `付与登録`
4. `有給残高（概算）` で登録後の残高を確認する

#### 入力項目/制約

- 付与登録には事前にプロファイル（基準日）が必要
- 付与理由は必須（監査ログ記録対象）
- `general_affairs` 非所属ユーザは付与APIを実行できません

![休暇](../test-results/2026-03-09-frontend-e2e-r1/22-leave-requests.png)

### 見積

- 目的: 見積ドラフトの作成と承認依頼
- 主な操作: 案件選択 → 金額/通貨/有効期限/備考 → 作成 → 承認依頼 → 送信（Stub）

### 詳細操作

1. 「案件選択」「金額」「通貨」「有効期限」「備考」を入力する
2. `作成` で見積ドラフトを作成する
3. `読み込み` で一覧を更新する
4. 一覧の `承認依頼` を押して承認フローに回す
5. 承認済みの見積は `送信 (Stub)` で送信状態にする
6. `詳細` を開き、送信履歴を `更新` で取得する

### 入力項目/制約

- `承認依頼` は `draft` のみ
- `送信 (Stub)` は `approved` または `sent` のみ

![見積](../test-results/2026-03-09-frontend-e2e-r1/05-core-estimates.png)

### 請求（ドラフト）

- 目的: 工数や金額から請求ドラフトを作成
- 主な操作: 案件選択 → 金額設定 → 作成、工数期間から作成、詳細確認
- 補足: 工数から作成した場合、対象工数は請求に紐づきます

### 詳細操作

1. 「案件」「金額」を入力し `作成` で請求ドラフトを作成する
2. `読み込み` で一覧を更新する
3. 「工数から請求ドラフト作成」で期間と単価を指定し `工数から作成`
4. 一覧の `詳細` を開き、送信履歴を `更新` で確認する
5. `送信 (Stub)` で送信状態にする
6. `draft` 状態では「工数リンク解除」が利用可能
7. `詳細` 内の `注釈` からメモ・参照リンクを更新できる（必要時）

### 入力項目/制約

- 工数から作成した場合、対象工数は解除するまで編集/付け替え不可

![請求ドラフト](../test-results/2026-03-09-frontend-e2e-r1/06-core-invoices.png)

### ルームチャット（案件ルーム）

- 目的: 案件ルームでのプロジェクト単位コミュニケーション
- 主な操作: ルーム選択、投稿、メンション、確認依頼（ack required）

### 詳細操作

1. 「ルームチャット」を開き、`ルーム` で `project: ...` の案件ルームを選択する
2. `再読込` で最新メッセージを取得する
3. 通知設定で全投稿/メンション/ミュートを調整する（任意）
4. `要約` を押して要約（スタブ）を取得する
5. 本文・タグ・添付を入力し `送信` を押す
6. メンションはユーザ/グループを補完から追加し、必要に応じて `@all` を選ぶ
7. 「確認依頼」では確認対象（ユーザ/グループ/ロール）と期限（任意）を入力し `確認依頼` を押す
8. 一覧で `OK` を押して確認済みにする
9. リアクションボタンでスタンプを付ける
10. `さらに読み込む` で過去メッセージを取得する
11. 親メッセージの `スレッドを開く` で返信履歴を表示する
12. スレッド内で返信本文・メンション・タグを入力し `返信` を押す
13. 確認が必要な返信は `確認依頼として返信` を選び、対象ユーザを指定して送信する
14. スレッド内の返信でもリアクション、`OK`、`OK取消`、確認依頼の撤回を利用できる
15. 横断検索で `返信` と表示された結果は `スレッドを開く` で親メッセージと一緒に確認する

### 補足（入力と取り消し）

- 添付は `ファイルを選択` から追加し、添付一覧から削除できます
- `OK取消` は即時確定ではなく、トーストの `取り消す` で一定時間取り消し可能です
- 削除済みの親・返信は本文を表示せずプレースホルダーを表示します。削除済みの親へ新しい返信は投稿できません
- スレッドを閉じるには `閉じる` または Escape キーを使います。閉じた後は起点のボタンへフォーカスが戻ります

### 確認依頼（ack required）

- 対象はユーザ/グループ/ロールで指定でき、投稿時に対象ユーザが確定する
- 期限は任意。期限超過は一覧に表示される
- `OK` で確認済み。`OK` の取り消しは本人のみ可能
- 確認依頼の取消は投稿者または管理者のみ可能
- 補足: 概要は [chat-guide](chat-guide.md) を参照

### 入力項目/制約

- メッセージ本文は 2000 文字まで
- `@all` は候補が許可されている場合のみ表示
- 通知設定は保存後の投稿から反映

![ルームチャット（案件ルーム）](../test-results/2026-03-09-frontend-e2e-r1/14-room-chat.png)

### ルームチャット（DM/Private Group）

- 目的: ルーム単位のコミュニケーション
- 主な操作: ルーム選択、投稿、メンション
- 総務グループ（`general_affairs`）に所属するユーザは、表示範囲を `総務相談のみ` に切り替えて個人総務チャットを inbox として確認できます
- 補足: DM/Private Group の利用可否は管理設定に依存します

### 詳細操作

1. 「ルーム」で対象ルームを選択し `再読込` を押す
2. （総務グループのみ）「表示範囲」を `総務相談のみ` に切り替えると、個人総務チャットのみ表示される
3. 「ルーム検索」でルーム名/種別で候補を絞り込む
4. `要約` を実行する（外部要約は許可済みルームのみ）
5. 「private_group 作成」で名称と初期メンバーを入力し作成する
6. 「DM 作成」で相手 userId を入力し作成する
7. private_group では「招待」でメンバーを追加する
8. 通知設定で全投稿/メンション/ミュートを調整する（任意）
9. 投稿欄で本文/タグ/添付/確認対象（ユーザ/グループ/ロール）を設定し `送信` / `確認依頼`
10. 一覧でフィルタを設定し `適用` / `クリア`
11. `さらに読み込む` で過去メッセージを取得する
12. 「横断検索（チャット全体）」で検索し `スレッドを開く` で該当親メッセージと返信を開く

### 入力項目/制約

- ルーム未選択時は投稿・フィルタ操作が無効
- 外部要約は `allowExternalIntegrations` が有効なルームのみ表示
- 通知設定は保存後の投稿から反映

![ルームチャット](../test-results/2026-03-09-frontend-e2e-r1/14-room-chat.png)

![ルームチャットのスレッド（375px mobile）](../test-results/2026-08-09-issue2014-chat-thread-ui/01-chat-thread-mobile.png)

### オフライン動作（参考）

- 目的: オフライン時の送信待ちと復旧後の再送
- 主な操作: オフライン保存 → 再送
- 補足: 送信待ちは「現在のユーザー > オフライン送信キュー」で確認

![オフライン送信待ち](../test-results/2026-03-09-frontend-e2e-r1/14-offline-daily-queue.png)
![オフライン再送](../test-results/2026-03-09-frontend-e2e-r1/15-offline-queue-retry.png)
![オフライン重複工数](../test-results/2026-03-09-frontend-e2e-r1/16-offline-duplicate-time-entry.png)

---

## 補足

- PWA のキャッシュ更新確認: サービスワーカー更新後の表示確認に利用します。

![PWA キャッシュ更新](../test-results/2026-03-09-frontend-e2e-r1/20-sw-cache-refresh.png)

- `pwa push subscribe flow` は `VITE_PUSH_PUBLIC_KEY` 未設定のため E2E ではスキップしています。
- 画面デザインは `@itdo/design-system` 適用済みです（compact density）。
