...
- 仕入請求に関連する発注書（PO）を設定/解除できる（同一 project/vendor の PO のみ候補）
- ステータスにより変更理由が必須になる（監査ログに記録）

#### 配賦明細（任意）

- 請求書PDFを参照し、必要な場合のみ配賦明細を入力する
- 税率別合計と請求合計の差分を確認し、端数はシステムで自動調整される
- 差分が解消できない場合は理由を添えて管理者へエスカレーションする
...
![見積](../test-results/2026-03-09-frontend-e2e-r1/05-core-estimates.png)

![請求ドラフト](../test-results/2026-03-09-frontend-e2e-r1/06-core-invoices.png)
