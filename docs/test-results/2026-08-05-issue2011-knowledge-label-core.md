...
- hidden、logical deleted、grant revoked、cross-domain、absent labelは同じ`404 not_found`で非列挙化する。delete時にmanage不能なactive childが含まれる場合は、具体的なchild拒否を返さずgeneric conflictとし、子row lockを待たない。
- reparentはACL修飾済みqueryでmanage可能なactive subtree rowだけをlockし、active subtree全件をmanageできない場合はpathを変更せずgeneric conflictとする。logical deleted descendantはlock対象外だがpath再構築には含め、provenanceとclosure整合性を維持する。self path、cycle、domain、最大depth 8、broken pathもtransaction内で検証する。
...
| focused Knowledge item + label tests          | PASS   | 81 tests、fail/skip/todo 0。Workstream 02 item回帰を含むlabel core focused suite                                                                               |
| PostgreSQL 15 integration                     | PASS   | fixed digest、tmpfs、loopback ephemeral port。`labels=18`、`paths=59`、`assignments=3`（active 2 / detached 1）、`labelAudits=27`、`maxDepth=8`                |
| migration deploy / status                     | PASS   | 空DBへ全migration適用後、schema up to date                                                                                                                     |
| hierarchy/ACL transaction checks              | PASS   | cycle、cross-domain、depth 8許可/depth 9拒否、active subtree/child全件manage、deleted descendant path保持、attach/logical detach、grant revoke、audit rollback |
| concurrency / lock non-enumeration            | PASS   | 同一slug、逆向きreparent、同時attach/detachを競合正規化。権限外label/item/subtree/active child requestはhidden row lockを待たずgeneric failure                 |
...
| focused source coverage                       | PASS   | source-map付き再build後のtarget 3 source files: statements/lines 75.05%、branches 67.16%、functions 76.54%、35 tests、fail/skip/todo 0                         |
...
| backend full test                                 | PASS    | 1,610 tests、fail/skip/todo 0                                                                     |
