...
- TTL付きDB auth context cacheはidentity-backed entryの期限を設定TTLと`UserIdentity.effectiveUntil`の早い方に制限する。identity有効中はcache hitを維持し、失効後はDBを再照会してKnowledge routeを401で拒否する。session cacheはcanonical identity/account単位で分離し、同じprovider subjectへ紐づく別identity/session間でactive/blocked contextを再利用しない。SCIMと手動group/membership mutationはDB mutation/transaction commit直後かつ外側監査処理を待つ前に全消去し、複数DB操作のmembership同期は途中失敗時にもcacheを消去する。identity/credentialの対象更新では同じprovider subjectのsession/JWT entryを一括無効化し、DB解決中にsubject/global invalidationが完了した場合はcache write epochで失効前snapshotの再挿入を拒否する。membership失効後のorganization read/createとgroup由来roleを同一processで再利用しない。分散invalidation未実装の複数backend instanceではTTL 0を要求する
...
| SCIM auth cache / personal GA transaction focused                 | PASS   | cache subprocess 6、identity side effects 9、personal GA lifecycle 7。revoke commit直後かつ外側監査待機前のcache失効、membership副作用失敗時のaccount無効化rollback、commit失敗時の副作用監査rollbackを確認                                                                                                          |
...
| `npm run test --prefix packages/backend`                          | PASS   | 1,558 tests、fail/skip/todo 0                                                                                                                                                                                                                                                                                        |
