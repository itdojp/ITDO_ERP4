...
- TTL付きDB auth context cacheはidentity-backed entryの期限を設定TTLと`UserIdentity.effectiveUntil`の早い方に制限する。identity有効中はcache hitを維持し、失効後はDBを再照会してKnowledge routeを401で拒否する。SCIMと手動group/membership mutationでも成功直後に全消去し、membership失効後のorganization read/createとgroup由来roleを同一processで再利用しない。分散invalidation未実装の複数backend instanceではTTL 0を要求する
...
| auth cache invalidation / expiry focused                          | PASS   | 4 subprocess tests: TTL有効時の手動membership削除、group rename/deactivate、`UserIdentity.effectiveUntil`前のcache hitと失効後のDB再照会/Knowledge 401。membership失効後Knowledge list/count/detail/createも確認                                                                             |
...
| `npm run test --prefix packages/backend`                          | PASS   | 1,551 tests、skip/todo 0                                                                                                                                                                                                                                                                     |
| `npm run coverage:auth:check --prefix packages/backend`           | PASS   | auth 153 tests。S/L 90.36%、B 70.96%、F 98.67%。既存閾値の低下なし                                                                                                                                                                                                                           |
