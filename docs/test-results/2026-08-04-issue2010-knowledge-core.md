...
- TTL付きDB auth context cacheはidentity-backed entryの期限を設定TTLと`UserIdentity.effectiveUntil`の早い方に制限する。identity有効中はcache hitを維持し、失効後はDBを再照会してKnowledge routeを401で拒否する。session cacheはcanonical identity/account単位で分離し、同じprovider subjectへ紐づく別identity/session間でactive/blocked contextを再利用しない。SCIMと手動group/membership mutationでも成功直後に全消去し、identity/credentialの対象更新では同じprovider subjectのsession/JWT entryを一括無効化する。DB解決中にsubject/global invalidationが完了した場合はcache write epochで失効前snapshotの再挿入を拒否し、membership失効後のorganization read/createとgroup由来roleを同一processで再利用しない。分散invalidation未実装の複数backend instanceではTTL 0を要求する
- SCIM user DELETEのaccount無効化と個人総務チャットmembership無効化は同じtransactionで確定する。副作用失敗時はaccount無効化もrollbackし、成功時はcommit直後かつ外側監査ログより先にauth context cacheを消去する
...
| SCIM auth cache / personal GA transaction focused                 | PASS   | cache subprocess 5、personal GA lifecycle 6。DELETE成功時のcommit直後cache失効、およびmembership副作用失敗時のaccount無効化rollbackと既存認証状態の整合を確認                                                                                                                                                        |
...
| `npm run test --prefix packages/backend`                          | PASS   | 1,556 tests、fail/skip/todo 0                                                                                                                                                                                                                                                                                        |
