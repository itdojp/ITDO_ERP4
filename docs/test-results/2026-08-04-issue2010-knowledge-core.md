...
- JWT/sessionのowner subjectにはstableな`UserAccount.id`を使用し、SCIM等で`externalId` / `userName`が変わってもownership predicateを維持する。header authはdevelopment/test用synthetic IDへ限定する
...
| ----------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| focused Knowledge tests                                           | PASS   | 40 tests: use case 18、Prisma adapter 9、route 9、schema/migration/OpenAPI 4。use case内でcredential root 18種 × encoding 4種 × 配置3種の216-case embedded-value matrixをcreate/update双方へ適用 |
...
| focused coverage                                                  | PASS   | adapter S/L/F 100%、B 87.01%；use case S/L 97.12%、B 88.20%、F 97.61%；route S/L 99.47%、B 77.77%、F 100%。閾値の追加・低下なし                                                                  |
...
| `npm run test --prefix packages/backend`                          | PASS   | 1,543 tests、skip/todo 0                                                                                                                                                                         |
