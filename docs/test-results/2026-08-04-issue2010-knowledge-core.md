...
| auth cache invalidation / expiry focused                          | PASS   | 5 subprocess tests: TTL有効時の手動membership削除、group rename/deactivate、JWT/sessionの`UserIdentity.effectiveUntil`前のcache hitと失効後のDB再照会/Knowledge 401、同一provider subjectの別session identity間で正・負cacheが相互流用されないこと。membership失効後Knowledge list/count/detail/createも確認                                                                             |
...
| `npm run test --prefix packages/backend`                          | PASS   | 1,552 tests、skip/todo 0                                                                                                                                                                                                                                                                     |
| `npm run coverage:auth:check --prefix packages/backend`           | PASS   | auth 153 tests。S/L 90.32%、B 70.70%、F 98.68%。既存閾値の低下なし                                                                                                                                                                                                                           |
