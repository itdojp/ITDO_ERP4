...
- canonical URLのuserinfo、安全なfragment、tracking queryを除去する一方、credential-like queryはrequest全体を拒否する。OAuth assertion/verifier/proof系を含むquery名と値を入力長でboundedに多重percent-decodeし、malformed prefix、先頭slashのないpath-relative URL、semicolon/encoded delimiter、入れ子およびtop-level hash-router fragment内のcredential query textを同じdecoderで検査する。解析上限到達はfail closedとし、正規化後の過大URLも拒否する
...
| focused coverage                                                  | PASS   | adapter S/L/F 100%、B 87.50%；use case S/L 96.28%、B 86.22%、F 97.22%；route S/L 99.47%、B 77.14%、F 100%。閾値の追加・低下なし    |
