...
- JWT/sessionのowner subjectにはstableな`UserAccount.id`を使用し、SCIM等で`externalId` / `userName`が変わってもownership predicateを維持する。non-header authでcanonical accountを解決できない場合はKnowledge route全体を403でfail closedとし、raw JWT subject / legacy keyをowner/auditへ保存しない。header authはdevelopment/test用synthetic IDへ限定する
...
| ----------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| focused Knowledge tests                                           | PASS   | 43 tests: use case 18、Prisma adapter 9、route 12、schema/migration/OpenAPI 4。use case内でcredential root 18種 × encoding 4種 × 配置3種の216-case embedded-value matrixをcreate/update双方へ適用 |
| auth plugin canonical-account integration                         | PASS   | DB未解決JWTで`GET /knowledge/items`が`403 canonical_account_required`となり、raw subjectをKnowledge serviceへ渡さないsubprocess test                                                              |
...
| focused coverage                                                  | PASS   | adapter S/L/F 100%、B 87.01%；use case S/L 97.12%、B 88.20%、F 97.61%；route S/L 99.52%、B 80.85%、F 100%。閾値の追加・低下なし                                                                   |
...
| `npm run test --prefix packages/backend`                          | PASS   | 1,547 tests、skip/todo 0                                                                                                                                                                          |
| `npm run coverage:auth:check --prefix packages/backend`           | PASS   | auth 152 tests。S/L 90.24%、B 71.00%、F 98.66%。既存閾値の低下なし                                                                                                                                |
