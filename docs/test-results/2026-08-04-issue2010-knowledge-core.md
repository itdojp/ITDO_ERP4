...
- JWT/sessionのowner subjectにはactiveな`UserIdentity`に裏付けられたstableな`UserAccount.id`を使用し、SCIM等で`externalId` / `userName`が変わってもownership predicateを維持する。non-header authでcanonical accountとidentityを解決できない場合はKnowledge route全体を403でfail closedとし、raw JWT subjectや既存API互換のlegacy `userName` / `externalId`解決結果をowner/auditへ保存しない。header authはdevelopment/test用synthetic IDへ限定する
- canonical URLのuserinfo、安全なfragment、tracking queryを除去する一方、credential-like queryはrequest全体を拒否する。署名URLの`key` / `policy` / `expires`、OAuthの`state` / `code`、password別名の`pwd` / `passphrase`は値が一見無害でもfail-closedで拒否する。OAuth assertion/verifier/proof系とSAML request/artifact/relay-state系を含むquery名を検査し、query値も入力長でboundedに多重percent-decodeして、先頭のcredential-like `name=value`を安全なouter query名の配下でも拒否する。malformed prefix、先頭slashのないpath-relative URL、semicolon/encoded delimiter、入れ子およびtop-level hash-router fragment内のcredential query textを同じdecoderで検査する。解析上限到達はfail closedとし、正規化後の過大URLも拒否する
...
- Fastify/Ajvがunknown body fieldを除去する前の`preValidation`でcreate/update/delete/restoreのraw field allowlistを検査し、owner/audit field等が混在するrequestを全体拒否する。service境界のallowlistも維持し、route迂回とsilent partial mutationの双方を防ぐ
...
- non-header Knowledge actorは`auth.userAccountId`だけでなくDB解決済み`auth.identityId`も必須とする。JWT subjectが既存`userName` / `externalId`と衝突してlegacy account解決が成立してもKnowledge routeでは403とし、既存非Knowledge APIの互換fallback自体は変更しない
- TTL付きDB auth context cacheはidentity-backed entryの期限を設定TTLと`UserIdentity.effectiveUntil`の早い方に制限する。identity有効中はcache hitを維持し、失効後はDBを再照会してKnowledge routeを401で拒否する。session cacheはcanonical identity/account単位で分離し、同じprovider subjectへ紐づく別identity/session間でactive/blocked contextを再利用しない。SCIMと手動group/membership mutationはDB mutation/transaction commit直後かつ外側監査処理を待つ前に全消去する。SCIM Group POST/PUT/PATCHではgroup本体と全membership操作を同じPrisma transactionへ束縛し、入力をmutation前に全件検証する。途中失敗時はgroup/membershipをrollbackして失敗操作の外側監査を残さず、成功commit後だけcacheを全消去する。identity/credentialの対象更新では同じprovider subjectのsession/JWT entryを一括無効化し、DB解決中にsubject/global invalidationが完了した場合はcache write epochで失効前snapshotの再挿入を拒否する。membership失効後のorganization read/createとgroup由来roleを同一processで再利用しない。分散invalidation未実装の複数backend instanceではTTL 0を要求する
- SCIM user DELETEのaccount無効化、個人総務チャットmembership無効化、その副作用監査は同じtransactionへ束縛する。chat副作用またはcommit失敗時はaccount/chat変更とtransaction内監査をrollbackし、成功時はcommit直後かつ外側監査ログより先にauth context cacheを消去する。監査書込み自体の失敗は既存fail-open契約を維持する。共通side-effect portを使うPUT/PATCHの副作用監査も同じtransaction clientへ束縛する
...
- audit metadataはscope/status/version/変更field名だけを保存し、本文、URL、token ID、secretを保存しない。actor provenanceの`userId`はapplication serviceが認可済みowner subjectから強制導出する。安全文字・128文字上限をadapterでも再検証したrequest ID、有限値`api|agent`のsourceだけを追加し、raw role/group display name/scope/IP/User-AgentはKnowledge audit port/DBへ渡さない
...
| ----------------------------------------------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| focused Knowledge tests                                           | PASS   | 45 tests: use case 19、Prisma adapter 9、route 13、schema/migration/OpenAPI 4。全7 routeのcanonical actor guardとpersonal scope + organization grantのno-write契約を追加。use case内でcredential root 18種 × encoding 4種 × 配置3種の216-case embedded-value matrixをcreate/update双方へ適用                         |
| auth plugin canonical-account integration                         | PASS   | DB未解決JWT、および`UserIdentity`未解決だがJWT subjectとlegacy account keyが衝突するJWTの双方で`GET /knowledge/items`が`403 canonical_account_required`。後者はlegacy account解決自体が成立したこともfixtureで確認                                                                                                   |
| auth cache invalidation / expiry focused                          | PASS   | 7 subprocess tests: TTL有効時の手動membership削除、group rename/deactivate、JWT/sessionの`UserIdentity.effectiveUntil`、同一provider subjectの別session identity間の正・負cache分離、DB照会中のsubject/global invalidation後に旧snapshotを再挿入しないこと。membership失効後Knowledge list/count/detail/createも確認 |
| SCIM auth cache / personal GA transaction focused                 | PASS   | cache subprocess 9、identity side effects 9、personal GA lifecycle 7。revoke commit直後かつ外側監査待機前のcache失効、SCIM Group POST/PUT/PATCHの原子的rollback、PATCH全member事前検証、membership副作用失敗時のaccount無効化rollback、commit失敗時の副作用監査rollbackを確認                                        |
| focused coverage                                                  | PASS   | adapter S/L/F 100%、B 87.01%；use case S/L 97.51%、B 88.47%、F 97.61%；route S/L 99.52%、B 84.00%、F 100%。閾値の追加・低下なし                                                                                                                                                                                      |
...
| `npm run test --prefix packages/backend`                          | PASS   | 1,561 tests、fail/skip/todo 0                                                                                                                                                                                                                                                                                        |
| `npm run coverage:auth:check --prefix packages/backend`           | PASS   | auth 153 tests。S/L 90.27%、B 70.73%、F 98.68%。既存閾値の低下なし                                                                                                                                                                                                                                                   |
