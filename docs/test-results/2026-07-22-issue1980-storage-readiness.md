...
| 検証                                                     | 結果 | 詳細                                                                           |
| -------------------------------------------------------- | ---- | ------------------------------------------------------------------------------ |
| storage readiness focused backend tests                  | PASS | 11 files／112 tests                                                            |
| focused changed-source coverage                          | PASS | statements/lines 87.86%、branches 81.85%、functions 93.61%。閾値低下／除外なし |
...
| `make test`                                              | PASS | backend 1,471 tests、frontend 82 files／468 tests                              |
...
backend full test中に、local PostgreSQLを起動していない既存vendor invoice監査testの非致命的なPrisma warningが出力されたが、suiteは1,471件すべて成功した。今回の監視pathはDBへ接続しない。
